...
NSString *const DGCYYTextViewTextDidBeginEditingNotification = @"DGCYYTextViewTextDidBeginEditing";
NSString *const DGCYYTextViewTextDidChangeNotification = @"DGCYYTextViewTextDidChange";
NSString *const DGCYYTextViewTextDidEndEditingNotification = @"DGCYYTextViewTextDidEndEditing";


typedef NS_ENUM (NSUInteger, DGCYYTextGrabberDirection) {
    kStart = 1,
    kEnd   = 2,
};

typedef NS_ENUM(NSUInteger, DGCYYTextMoveDirection) {
    kLeft   = 1,
    kTop    = 2,
    kRight  = 3,
    kBottom = 4,
...
@interface _DGCYYTextViewUndoObject : NSObject
@property (nonatomic, strong) NSAttributedString *text;
@property (nonatomic, assign) NSRange selectedRange;
@end
@implementation _DGCYYTextViewUndoObject
+ (instancetype)objectWithText:(NSAttributedString *)text range:(NSRange)range {
    _DGCYYTextViewUndoObject *obj = [self new];
...
@interface DGCYYTextView () <UIScrollViewDelegate, DGCYYTextDebugTarget, DGCYYTextKeyboardObserver> {
    
    DGCYYTextRange *_selectedTextRange; /// nonnull
    DGCYYTextRange *_markedTextRange;
    
    __weak id<DGCYYTextViewDelegate> _outerDelegate;
    ...
    DGCYYTextContainer *_innerContainer; ///< nonnull, inner text container
    DGCYYTextLayout *_innerLayout; ///< inner text layout, the text in this layout is longer than `_innerText` by appending '\n'
    
    DGCYYTextContainerView *_containerView; ///< nonnull
    DGCYYTextSelectionView *_selectionView; ///< nonnull
    DGCYYTextMagnifier *_magnifierCaret; ///< nonnull
    DGCYYTextMagnifier *_magnifierRanged; ///< nonnull
    ...
    DGCYYTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
    DGCYYTextLayout *_highlightLayout; ///< when _state.showingHighlight=YES, this layout should be displayed
    DGCYYTextRange *_trackingRange; ///< the range in _innerLayout, may out of _innerText.
    ...
    struct {
        unsigned int trackingGrabber : 2;       ///< DGCYYTextGrabberDirection, current tracking grabber
        ...
        unsigned int touchMoved : 3;            ///< DGCYYTextMoveDirection, move direction after touch began
...
@synthesize selectedTextRange = _selectedTextRange;  //copy nonnull (DGCYYTextRange*)
@synthesize markedTextRange = _markedTextRange;      //readonly     (DGCYYTextRange*)
...
- (void)_commitUpdate {
    ...
    [[DGCYYTextTransaction transactionWithTarget:self selector:@selector(_updateIfNeeded)] commit];
#else
    [self _update];
#endif
}
...
- (void)_updateLayout {
    ...
    [text removeAttribute:DGCYYTextBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:DGCYYTextBackgroundBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    ...
    _innerLayout = [DGCYYTextLayout layoutWithContainer:_innerContainer text:text];
...
- (void)_updateSelectionView {
    ...
    [[DGCYYTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    if (!_innerLayout) return;
    
    NSMutableArray *allRects = [NSMutableArray new];
    BOOL containsDot = NO;
    
    DGCYYTextRange *selectedRange = _selectedTextRange;
    ...
    [allRects enumerateObjectsUsingBlock:^(DGCYYTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
    ...
    if (!_state.firstShowDot && containsDot) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[DGCYYTextEffectWindow sharedWindow] showSelectionDot:self->_selectionView];
        });
    }
    [[DGCYYTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
...
- (void)_commitPlaceholderUpdate {
    ...
    [[DGCYYTextTransaction transactionWithTarget:self selector:@selector(_updatePlaceholderIfNeeded)] commit];
#else
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        DGCYYTextContainer *container = _innerContainer.copy;
        container.size = self.bounds.size;
        container.truncationType = DGCYYTextTruncationTypeEnd;
        container.truncationToken = nil;
        DGCYYTextLayout *layout = [DGCYYTextLayout layoutWithContainer:container text:_placeholderAttributedText];
...
- (void)_updateTextRangeByTrackingCaret {
    ...
    DGCYYTextPosition *newPos = [_innerLayout closestPositionToPoint:trackingPoint];
    if (newPos) {
        ...
        DGCYYTextRange *newRange = [DGCYYTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
...
- (void)_updateTextRangeByTrackingGrabber {
    ...
    DGCYYTextPosition *position = [_innerLayout positionForPoint:magPoint
    ...
    if (position) {
        ...
        if ((NSUInteger)position.offset > _innerText.length) {
            position = [DGCYYTextPosition positionWithOffset:_innerText.length];
        }
        DGCYYTextRange *newRange = [DGCYYTextRange rangeWithStart:(isStart ? position : _selectedTextRange.start)
...
- (void)_updateTextRangeByTrackingPreSelect {
    if (!_state.trackingTouch) return;
    DGCYYTextRange *newRange = [self _getClosestTokenRangeAtPoint:_trackingPoint];
    _trackingRange = newRange;
}

/// Show or update `_magnifierCaret` based on `_trackingPoint`, and hide `_magnifierRange`.
- (void)_showMagnifierCaret {
    if (DGCYYTextIsAppExtension()) return;
    
    if (_state.showingMagnifierRanged) {
        _state.showingMagnifierRanged = NO;
        [[DGCYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    }
    ...
    if (!_state.showingMagnifierCaret) {
        ...
        [[DGCYYTextEffectWindow sharedWindow] showMagnifier:_magnifierCaret];
    } else {
        [[DGCYYTextEffectWindow sharedWindow] moveMagnifier:_magnifierCaret];
    }
}
...
- (void)_showMagnifierRanged {
    ...
    if (_state.showingMagnifierCaret) {
        ...
        [[DGCYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    }
    ...
    if (lineIndex < _innerLayout.lines.count) {
        DGCYYTextLine *line = _innerLayout.lines[lineIndex];
        ...
        if (!_state.showingMagnifierRanged) {
            ...
            [[DGCYYTextEffectWindow sharedWindow] showMagnifier:_magnifierRanged];
        } else {
            [[DGCYYTextEffectWindow sharedWindow] moveMagnifier:_magnifierRanged];
        }
    }
}
...
- (void)_showMenu {
    ...
    if (_selectionView.caretVisible) {
        ...
        DGCYYTextSelectionRect *sRect = _selectionView.selectionRects.firstObject;
        rect = sRect.rect;
        ...
        DGCYYTextKeyboardManager *mgr = [DGCYYTextKeyboardManager defaultManager];
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _highlightLayout = [DGCYYTextLayout layoutWithContainer:_innerContainer text:hiText];
...
- (void)_scrollRangeToVisible:(DGCYYTextRange *)range {
    if (!range) return;
...
- (void)_keyboardChanged {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([DGCYYTextKeyboardManager defaultManager].keyboardVisible) {
...
- (void)_startLongPressTimer {
    ...
    _longPressTimer = [NSTimer timerWithTimeInterval:kLongPressMinimumDuration
                                              target:[DGCYYTextWeakProxy proxyWithTarget:self]
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if (_highlight.longPressAction) {
            ...
            CGRect rect = [_innerLayout rectForRange:[DGCYYTextRange rangeWithRange:_highlightRange]];
            rect = [self _convertRectFromLayout:rect];
        ...
        if (_state.trackingTouch) {
            if (_state.trackingGrabber) {
                ...
                if (newPos) {
                    ...
                    _trackingRange = [DGCYYTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
                    [self _updateSelectionView];
                }
                [self _hideMenu];
...
- (void)_fixSelectionDot {
    if (DGCYYTextIsAppExtension()) return;
    CGPoint origin = [self yy_convertPoint:CGPointZero toViewOrWindow:[DGCYYTextEffectWindow sharedWindow]];
...
- (DGCYYTextRange *)_getClosestTokenRangeAtPosition:(DGCYYTextPosition *)position {
    position = [self _correctedTextPosition:position];
    if (!position) return nil;
    DGCYYTextRange *range = nil;
    if (self.tokenizer) {
    ...
    if (!range || range.asRange.length == 0) {
        ...
        DGCYYTextRange *extStart = [_innerLayout textRangeByExtendingPosition:range.start];
        DGCYYTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:range.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            range = [DGCYYTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    range = [self _correctedTextRange:range];
    if (range.asRange.length == 0) {
        range = [DGCYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
/// Try to get the character range/position with word granularity from the tokenizer.
- (DGCYYTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point {
    point = [self _convertPointToLayout:point];
    DGCYYTextRange *touchRange = [_innerLayout closestTextRangeAtPoint:point];
    touchRange = [self _correctedTextRange:touchRange];
    
    if (self.tokenizer && touchRange) {
        DGCYYTextRange *encEnd = (id)[self.tokenizer rangeEnclosingPosition:touchRange.end withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionBackward];
        DGCYYTextRange *encStart = (id)[self.tokenizer rangeEnclosingPosition:touchRange.start withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionForward];
        if (encEnd && encStart) {
            NSArray *arr = [@[encEnd.start, encEnd.end, encStart.start, encStart.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [DGCYYTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    if (touchRange) {
        DGCYYTextRange *extStart = [_innerLayout textRangeByExtendingPosition:touchRange.start];
        DGCYYTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:touchRange.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [DGCYYTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    ...
    if (!touchRange) touchRange = [DGCYYTextRange defaultRange];
    
    if (_innerText.length && touchRange.asRange.length == 0) {
        touchRange = [DGCYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
- (DGCYYTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    if (!_highlightable || !_innerLayout.containsHighlight) return nil;
    point = [self _convertPointToLayout:point];
    DGCYYTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    DGCYYTextHighlight *highlight = [text attribute:DGCYYTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGFloat)_getMagnifierRangedOffset {
    ...
    DGCYYTextPosition *position = [_innerLayout closestPositionToPoint:magPoint];
    NSUInteger lineIndex = [_innerLayout lineIndexForPosition:position];
    if (lineIndex < _innerLayout.lines.count) {
        DGCYYTextLine *line = _innerLayout.lines[lineIndex];
        if (_verticalForm) {
...
/// Return a DGCYYTextMoveDirection from `_touchBeganPoint` to `_trackingPoint`.
...
- (BOOL)_isTextPositionValid:(DGCYYTextPosition *)position {
    if (!position) return NO;
    if (position.offset < 0) return NO;
    if (position.offset > _innerText.length) return NO;
    if (position.offset == 0 && position.affinity == DGCYYTextAffinityBackward) return NO;
    if (position.offset == _innerText.length && position.affinity == DGCYYTextAffinityBackward) return NO;
    return YES;
}

/// Whether the range is valid (not out of bounds).
- (BOOL)_isTextRangeValid:(DGCYYTextRange *)range {
...
- (DGCYYTextPosition *)_correctedTextPosition:(DGCYYTextPosition *)position {
    if (!position) return nil;
    if ([self _isTextPositionValid:position]) return position;
    if (position.offset < 0) {
        return [DGCYYTextPosition positionWithOffset:0];
    }
    if (position.offset > _innerText.length) {
        return [DGCYYTextPosition positionWithOffset:_innerText.length];
    }
    if (position.offset == 0 && position.affinity == DGCYYTextAffinityBackward) {
        return [DGCYYTextPosition positionWithOffset:position.offset];
    }
    if (position.offset == _innerText.length && position.affinity == DGCYYTextAffinityBackward) {
        return [DGCYYTextPosition positionWithOffset:position.offset];
    }
    return position;
}
...
- (DGCYYTextRange *)_correctedTextRange:(DGCYYTextRange *)range {
    if (!range) return nil;
    if ([self _isTextRangeValid:range]) return range;
    DGCYYTextPosition *start = [self _correctedTextPosition:range.start];
    DGCYYTextPosition *end = [self _correctedTextPosition:range.end];
    return [DGCYYTextRange rangeWithStart:start end:end];
}
...
- (void)_replaceRange:(DGCYYTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    if (NSEqualRanges(range.asRange, _selectedTextRange.asRange)) {
        ...
        _selectedTextRange = [DGCYYTextRange rangeWithRange:newRange];
        ...
        if (range.asRange.length != text.length) {
            ...
            if (unionRange.length == 0) {
                ...
                _selectedTextRange = [DGCYYTextRange rangeWithRange:NSMakeRange(range.start.offset + text.length, 0)];
...
- (void)_updateAttributesHolder {
    if (_innerText.length > 0) {
        ...
        [_typingAttributesHolder removeAttribute:DGCYYTextBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:DGCYYTextBackgroundBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
    }
}
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (self->_highlightTextAttributes.count) {
                    DGCYYTextHighlight *highlight = [DGCYYTextHighlight highlightWithAttributes:self->_highlightTextAttributes];
...
- (UIViewController *)_getRootViewController {
    ...
    UIApplication *app = DGCYYTextSharedApplication();
...
- (void)_resetUndoAndRedoStack {
    ...
    _DGCYYTextViewUndoObject *object = [_DGCYYTextViewUndoObject objectWithText:_innerText.copy range:_selectedTextRange.asRange];
...
- (void)_saveToUndoStack {
    ...
    _DGCYYTextViewUndoObject *lastObject = _undoStack.lastObject;
...
- (void)_saveToRedoStack {
    if (!_allowsUndoAndRedo) return;
    _DGCYYTextViewUndoObject *lastObject = _redoStack.lastObject;
    if ([lastObject.text isEqualToAttributedString:self.attributedText]) return;
...
- (BOOL)_canUndo {
    ...
    _DGCYYTextViewUndoObject *object = _undoStack.lastObject;
...
- (void)_redo {
    ...
    _DGCYYTextViewUndoObject *object = _redoStack.lastObject;
...
- (void)_setLinePositionModifier:(id<DGCYYTextLinePositionModifier>)linePositionModifier {
...
- (void)_initTextView {
    ...
    // UITextInput
    _selectedTextRange = [DGCYYTextRange defaultRange];
    ...
    _innerContainer = [DGCYYTextContainer new];
    ...
    DGCYYTextBorder * border = [DGCYYTextBorder new];
    ...
    _containerView = [DGCYYTextContainerView new];
    _containerView.hostView = self;
    
    _selectionView = [DGCYYTextSelectionView new];
    _selectionView.userInteractionEnabled = NO;
    _selectionView.hostView = self;
    _selectionView.color = [self _defaultTintColor];
    
    _magnifierCaret = [DGCYYTextMagnifier magnifierWithType:DGCYYTextMagnifierTypeCaret];
    _magnifierCaret.hostView = _containerView;
    _magnifierRanged = [DGCYYTextMagnifier magnifierWithType:DGCYYTextMagnifierTypeRanged];
    ...
    self.debugOption = [DGCYYTextDebugOption sharedDebugOption];
    [DGCYYTextDebugOption addDebugTarget:self];
    ...
    [[DGCYYTextKeyboardManager defaultManager] addObserver:self];
...
- (void)dealloc {
    ...
    [[DGCYYTextKeyboardManager defaultManager] removeObserver:self];
    
    [[DGCYYTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    [[DGCYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    [[DGCYYTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    
    [DGCYYTextDebugOption removeDebugTarget:self];
...
- (void)scrollRangeToVisible:(NSRange)range {
    DGCYYTextRange *textRange = [DGCYYTextRange rangeWithRange:range];
    textRange = [self _correctedTextRange:textRange];
...
- (void)setText:(NSString *)text {
    ...
    [self replaceRange:[DGCYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:text];
}
...
- (void)setDataDetectorTypes:(UIDataDetectorTypes)dataDetectorTypes {
    ...
    NSTextCheckingType type = DGCYYTextNSTextCheckingTypeFromUIDataDetectorType(dataDetectorTypes);
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (text.length == 0) {
        [self replaceRange:[DGCYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:@""];
        return;
    }
    ...
    [_inputDelegate textWillChange:self];
     _innerText = text;
    _selectedTextRange = [DGCYYTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:DGCYYTextViewTextDidChangeNotification object:self];
...
- (void)setSelectedRange:(NSRange)selectedRange {
    ...
    DGCYYTextRange *range = [DGCYYTextRange rangeWithRange:selectedRange];
    range = [self _correctedTextRange:range];
    [self _endTouchTracking];
...
- (DGCYYTextDebugOption *)debugOption {
    return _containerView.debugOption;
}

- (DGCYYTextLayout *)textLayout {
...
- (CGSize)sizeThatFits:(CGSize)size {
    if (!_verticalForm && size.width <= 0) size.width = DGCYYTextContainerMaxSize.width;
    if (_verticalForm && size.height <= 0) size.height = DGCYYTextContainerMaxSize.height;
    ...
    if (!_verticalForm) {
        size.height = DGCYYTextContainerMaxSize.height;
    } else {
        size.width = DGCYYTextContainerMaxSize.width;
    }
    
    DGCYYTextContainer *container = [_innerContainer copy];
    container.size = size;
    
    DGCYYTextLayout *layout = [DGCYYTextLayout layoutWithContainer:container text:_innerText];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            DGCYYTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
            ...
            if (_state.trackingCaret) {
                ...
                if (!_state.touchMoved) {
                    if (_state.selectedWithoutEdit) {
                        ...
                        if (self.isFirstResponder) {
                            DGCYYTextRange *_oldRange = _trackingRange;
                            [self _updateTextRangeByTrackingCaret];
        ...
        if (_allowsTapAndSelect && _selectedTextRange.asRange.length == 0 && _innerText.length > 0) {
            DGCYYTextRange *newRange = [self _getClosestTokenRangeAtPosition:_selectedTextRange.start];
...
- (BOOL)becomeFirstResponder {
    ...
    if (!isFirstResponder && become) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DGCYYTextViewTextDidBeginEditingNotification object:self];
    }
    return become;
}
...
- (BOOL)resignFirstResponder {
    ...
    if (resign) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DGCYYTextViewTextDidEndEditingNotification object:self];
    }
    return resign;
}
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        ...
        Class cls = NSClassFromString(@"DGCYYImage");
        if (cls) {
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wundeclared-selector"
            if (p.yy_GIFData) {
...
#pragma clang diagnostic pop
        ...
        if (!img && p.yy_ImageData) {
            img = [UIImage imageWithData:p.yy_ImageData scale:DGCYYTextScreenScale()];
        }
            ...
            if (cls) {
                if ([img conformsToProtocol:NSProtocolFromString(@"YYAnimatedImage")]) {
                    ...
                    if (frameCount.integerValue > 1) {
                        Class viewCls = NSClassFromString(@"DGCYYAnimatedImageView");
    ...
    if (atr) {
        ...
        DGCYYTextPosition *pos = [self _correctedTextPosition:[DGCYYTextPosition positionWithOffset:endPosition]];
        DGCYYTextRange *range = [_innerLayout textRangeByExtendingPosition:pos];
        range = [self _correctedTextRange:range];
        if (range) {
...
- (id<DGCYYTextViewDelegate>)delegate {
    return _outerDelegate;
}

- (void)setDelegate:(id<DGCYYTextViewDelegate>)delegate {
...
#pragma mark - @protocol DGCYYTextKeyboardObserver

- (void)keyboardChangedWithTransition:(DGCYYTextKeyboardTransition)transition {
    [self _keyboardChanged];
}
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        DGCYYTextBinding *binding = [_innerText attribute:DGCYYTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
        if (binding && binding.deleteConfirm) {
            _state.deleteConfirm = YES;
            [_inputDelegate selectionWillChange:self];
            _selectedTextRange = [DGCYYTextRange rangeWithRange:effectiveRange];
            _selectedTextRange = [self _correctedTextRange:_selectedTextRange];
            [_inputDelegate selectionDidChange:self];
    ...
    _state.deleteConfirm = NO;
    if (range.length == 0) {
        DGCYYTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:_selectedTextRange.end inDirection:UITextLayoutDirectionLeft offset:1];
    ...
    [self replaceRange:[DGCYYTextRange rangeWithRange:range] withText:@""];
}
...
- (void)setSelectedTextRange:(DGCYYTextRange *)selectedTextRange {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        _markedTextRange = [DGCYYTextRange rangeWithRange:NSMakeRange(_selectedTextRange.end.offset, markedText.length)];
        if (_selectedTextRange.end.offset <= _innerText.length) {
            [_innerText replaceCharactersInRange:NSMakeRange(_selectedTextRange.end.offset, 0) withString:markedText];
        }
        _selectedTextRange = [DGCYYTextRange rangeWithRange:NSMakeRange(_selectedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    } else {
        ...
        _markedTextRange = [DGCYYTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset, markedText.length)];
        _selectedTextRange = [DGCYYTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    }
...
- (void)replaceRange:(DGCYYTextRange *)range withText:(NSString *)text {
    if (!range) return;
    if (!text) text = @"";
...
- (void)setBaseWritingDirection:(UITextWritingDirection)writingDirection forRange:(DGCYYTextRange *)range {
    if (!range) return;
    range = [self _correctedTextRange:range];
...
- (NSString *)textInRange:(DGCYYTextRange *)range {
    range = [self _correctedTextRange:range];
    if (!range) return @"";
    return [_innerText.string substringWithRange:range.asRange];
}

- (UITextWritingDirection)baseWritingDirectionForPosition:(DGCYYTextPosition *)position inDirection:(UITextStorageDirection)direction {
    [self _updateIfNeeded];
    position = [self _correctedTextPosition:position];
...
- (DGCYYTextPosition *)beginningOfDocument {
    return [DGCYYTextPosition positionWithOffset:0];
}

- (DGCYYTextPosition *)endOfDocument {
    return [DGCYYTextPosition positionWithOffset:_innerText.length];
}

- (DGCYYTextPosition *)positionFromPosition:(DGCYYTextPosition *)position offset:(NSInteger)offset {
    ...
    if (newLocation != 0 && newLocation != _innerText.length) {
        // fix emoji
        [self _updateIfNeeded];
        DGCYYTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:[DGCYYTextPosition positionWithOffset:newLocation]];
    ...
    DGCYYTextPosition *p = [DGCYYTextPosition positionWithOffset:newLocation];
    return [self _correctedTextPosition:p];
}

- (DGCYYTextPosition *)positionFromPosition:(DGCYYTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    [self _updateIfNeeded];
    DGCYYTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:offset];
    ...
    DGCYYTextPosition *newPosition = forward ? range.end : range.start;
    if (newPosition.offset > _innerText.length) {
        newPosition = [DGCYYTextPosition positionWithOffset:_innerText.length affinity:DGCYYTextAffinityBackward];
    }
    
    return [self _correctedTextPosition:newPosition];
}

- (DGCYYTextRange *)textRangeFromPosition:(DGCYYTextPosition *)fromPosition toPosition:(DGCYYTextPosition *)toPosition {
    return [DGCYYTextRange rangeWithStart:fromPosition end:toPosition];
}

- (NSComparisonResult)comparePosition:(DGCYYTextPosition *)position toPosition:(DGCYYTextPosition *)other {
    return [position compare:other];
}

- (NSInteger)offsetFromPosition:(DGCYYTextPosition *)from toPosition:(DGCYYTextPosition *)toPosition {
    return toPosition.offset - from.offset;
}

- (DGCYYTextPosition *)positionWithinRange:(DGCYYTextRange *)range farthestInDirection:(UITextLayoutDirection)direction {
    NSRange nsRange = range.asRange;
    if (direction == UITextLayoutDirectionLeft | direction == UITextLayoutDirectionUp) {
        return [DGCYYTextPosition positionWithOffset:nsRange.location];
    } else {
        return [DGCYYTextPosition positionWithOffset:nsRange.location + nsRange.length affinity:DGCYYTextAffinityBackward];
    }
}

- (DGCYYTextRange *)characterRangeByExtendingPosition:(DGCYYTextPosition *)position inDirection:(UITextLayoutDirection)direction {
    [self _updateIfNeeded];
    DGCYYTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:1];
    return [self _correctedTextRange:range];
}
...
- (DGCYYTextPosition *)closestPositionToPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    DGCYYTextPosition *position = [_innerLayout closestPositionToPoint:point];
    return [self _correctedTextPosition:position];
}

- (DGCYYTextPosition *)closestPositionToPoint:(CGPoint)point withinRange:(DGCYYTextRange *)range {
    DGCYYTextPosition *pos = (id)[self closestPositionToPoint:point];
    if (!pos) return nil;
...
- (DGCYYTextRange *)characterRangeAtPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    DGCYYTextRange *r = [_innerLayout closestTextRangeAtPoint:point];
...
- (CGRect)caretRectForPosition:(DGCYYTextPosition *)position {
    ...
    if (!CGRectIsNull(caretRect)) {
        ...
        return DGCYYTextCGRectPixelRound(caretRect);
    }
    return CGRectZero;
}
...
- (NSArray *)selectionRectsForRange:(DGCYYTextRange *)range {
    ...
    [rects enumerateObjectsUsingBlock:^(DGCYYTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
        rect.rect = [self _convertRectFromLayout:rect.rect];
    }];
    return rects;
}
...
- (UITextStorageDirection)selectionAffinity {
    if (_selectedTextRange.end.affinity == DGCYYTextAffinityForward) {
...
- (void)setSelectionAffinity:(UITextStorageDirection)selectionAffinity {
    _selectedTextRange = [DGCYYTextRange rangeWithRange:_selectedTextRange.asRange affinity:selectionAffinity == UITextStorageDirectionForward ? DGCYYTextAffinityForward : DGCYYTextAffinityBackward];
    [self _updateSelectionView];
}

- (NSDictionary *)textStylingAtPosition:(DGCYYTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
...
- (DGCYYTextPosition *)positionWithinRange:(DGCYYTextRange *)range atCharacterOffset:(NSInteger)offset {
    if (!range) return nil;
    ...
    else return [DGCYYTextPosition positionWithOffset:offset];
}

- (NSInteger)characterOffsetOfPosition:(DGCYYTextPosition *)position withinRange:(DGCYYTextRange *)range {
...
@interface DGCYYTextView(IBInspectableProperties)
@end

@implementation DGCYYTextView(IBInspectableProperties)
...
- (void)setDebugEnabled_:(BOOL)enabled {
    if (!enabled) {
        ...
        DGCYYTextDebugOption *debugOption = [DGCYYTextDebugOption new];
