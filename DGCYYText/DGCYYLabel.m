...
@interface DGCYYLabel() <DGCYYTextDebugTarget, DGCYYTextAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText; ///< nonnull
    DGCYYTextLayout *_innerLayout;
    DGCYYTextContainer *_innerContainer; ///< nonnull
    ...
    DGCYYTextLayout *_shrinkInnerLayout;
    DGCYYTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [DGCYYTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [DGCYYLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    DGCYYTextLayout *layout = _innerLayout;
    _innerLayout = nil;
...
- (DGCYYTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (DGCYYTextLayout *)_shrinkLayoutWithLayout:(DGCYYTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        DGCYYTextContainer *container = layout.container.copy;
        container.maximumNumberOfRows = 1;
        CGSize containerSize = container.size;
        if (!container.verticalForm) {
            containerSize.height = DGCYYTextContainerMaxSize.height;
        } else {
            containerSize.width = DGCYYTextContainerMaxSize.width;
        }
        container.size = containerSize;
        return [DGCYYTextLayout layoutWithContainer:container text:layout.text];
    } else {
        return nil;
    }
}
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        DGCYYTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        DGCYYTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
            DGCYYTextPosition *start = [DGCYYTextPosition positionWithOffset:_highlightRange.location];
            DGCYYTextPosition *end = [DGCYYTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:DGCYYTextAffinityBackward];
            DGCYYTextRange *range = [DGCYYTextRange rangeWithStart:start end:end];
...
- (DGCYYTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    DGCYYTextHighlight *highlight = [_innerText attribute:DGCYYTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _shrinkHighlightLayout = [DGCYYLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            case DGCYYTextTruncationTypeStart: {
                _lineBreakMode = NSLineBreakByTruncatingHead;
            } break;
            case DGCYYTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case DGCYYTextTruncationTypeMiddle: {
...
- (void)_initLabel {
    ((DGCYYTextAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [DGCYYTextDebugOption sharedDebugOption];
    [DGCYYTextDebugOption addDebugTarget:self];
    
    _font = [self _defaultFont];
    _textColor = [UIColor blackColor];
    _textVerticalAlignment = DGCYYTextVerticalAlignmentCenter;
    _numberOfLines = 1;
    _textAlignment = NSTextAlignmentNatural;
    _lineBreakMode = NSLineBreakByTruncatingTail;
    _innerText = [NSMutableAttributedString new];
    _innerContainer = [DGCYYTextContainer new];
    _innerContainer.truncationType = DGCYYTextTruncationTypeEnd;
...
+ (Class)layerClass {
    return [DGCYYTextAsyncLayer class];
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if ((!_verticalForm && size.width == self.bounds.size.width) ||
        ...
        [self _updateIfNeeded];
        DGCYYTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    DGCYYTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
    if (innerContainer) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            DGCYYTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                DGCYYTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    DGCYYTextPosition *start = [DGCYYTextPosition positionWithOffset:_highlightRange.location];
                    NSInteger endOffset = MIN(_innerLayout.visibleRange.length, _highlightRange.location + _highlightRange.length);
                    DGCYYTextPosition *end = [DGCYYTextPosition positionWithOffset:endOffset affinity:DGCYYTextAffinityBackward];
                    DGCYYTextRange *range = [DGCYYTextRange rangeWithStart:start end:end];
                    CGRect rect = [self._innerLayout rectForRange:range];
                    rect = [self _convertRectFromLayout:rect];
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    switch (lineBreakMode) {
        ...
        case NSLineBreakByCharWrapping:
        case NSLineBreakByClipping: {
            _innerContainer.truncationType = DGCYYTextTruncationTypeNone;
            _innerText.yy_lineBreakMode = lineBreakMode;
        } break;
        case NSLineBreakByTruncatingHead:{
            _innerContainer.truncationType = DGCYYTextTruncationTypeStart;
            _innerText.yy_lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        case NSLineBreakByTruncatingTail:{
            _innerContainer.truncationType = DGCYYTextTruncationTypeEnd;
        ...
        case NSLineBreakByTruncatingMiddle: {
            _innerContainer.truncationType = DGCYYTextTruncationTypeMiddle;
            _innerText.yy_lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        default: break;
    }
...
- (void)setTextLayout:(DGCYYTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((DGCYYTextAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (CGSize)intrinsicContentSize {
    if (_preferredMaxLayoutWidth == 0) {
        if (CGRectEqualToRect(_innerLayout.textBoundingRect, CGRectZero) == false) {
            return _innerLayout.textBoundingSize;
        }
        DGCYYTextContainer *container = [_innerContainer copy];
        container.size = DGCYYTextContainerMaxSize;
...
#pragma mark - DGCYYTextDebugTarget
...
#pragma mark - DGCYYTextAsyncLayerDelegate

- (DGCYYTextAsyncLayerDisplayTask *)newAsyncDisplayTask {
    
    // capture current context
    BOOL contentsNeedFade = _state.contentsNeedFade;
    NSAttributedString *text = _innerText;
    DGCYYTextContainer *container = _innerContainer;
    DGCYYTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    DGCYYTextDebugOption *debug = _debugOption;
    ...
    __block DGCYYTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block DGCYYTextLayout *shrinkLayout = nil;
    ...
    // create display task
    DGCYYTextAsyncLayerDisplayTask *task = [DGCYYTextAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        DGCYYTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [DGCYYTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [DGCYYLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        if (verticalAlignment == DGCYYTextVerticalAlignmentCenter) {
        ...
        } else if (verticalAlignment == DGCYYTextVerticalAlignmentBottom) {
            if (drawLayout.container.isVerticalForm) {
        ...
        point = DGCYYTextCGPointPixelRound(point);
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (DGCYYTextAttachment *a in drawLayout.attachments) {
        ...
        __strong DGCYYLabel *view = (DGCYYLabel *)layer.delegate;
        if (!view) return;
...
@interface DGCYYLabel(IBInspectableProperties)
@end

@implementation DGCYYLabel (IBInspectableProperties)
