...
static int _DGCYYTextKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _DGCYYTextKeyboardViewFrameObserver : NSObject
...
@implementation _DGCYYTextKeyboardViewFrameObserver {
    __unsafe_unretained UIView *_dgc_keyboardView;
}
- (void)addToKeyboardView:(UIView *)keyboardView {
    if (_dgc_keyboardView == keyboardView) return;
    if (_dgc_keyboardView) {
        [self removeFrameObserver];
        objc_setAssociatedObject(_dgc_keyboardView, &_DGCYYTextKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _dgc_keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_DGCYYTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)removeFrameObserver {
    [_dgc_keyboardView removeObserver:self forKeyPath:@"frame"];
    [_dgc_keyboardView removeObserver:self forKeyPath:@"center"];
    [_dgc_keyboardView removeObserver:self forKeyPath:@"bounds"];
    [_dgc_keyboardView removeObserver:self forKeyPath:@"transform"];
    _dgc_keyboardView = nil;
}

- (void)addFrameObserver {
    if (!_dgc_keyboardView) return;
    [_dgc_keyboardView addObserver:self forKeyPath:@"frame" options:kNilOptions context:NULL];
    [_dgc_keyboardView addObserver:self forKeyPath:@"center" options:kNilOptions context:NULL];
    [_dgc_keyboardView addObserver:self forKeyPath:@"bounds" options:kNilOptions context:NULL];
    [_dgc_keyboardView addObserver:self forKeyPath:@"transform" options:kNilOptions context:NULL];
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_DGCYYTextKeyboardViewFrameObserverKey);
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    if (_notifyBlock) {
        _notifyBlock(_dgc_keyboardView);
    }
}
...
@implementation DGCYYTextKeyboardManager {
    NSHashTable *_dgc_observers;
    
    CGRect _dgc_fromFrame;
    BOOL _dgc_fromVisible;
    UIInterfaceOrientation _dgc_fromOrientation;
    
    CGRect _dgc_notificationFromFrame;
    CGRect _dgc_notificationToFrame;
    NSTimeInterval _dgc_notificationDuration;
    UIViewAnimationCurve _dgc_notificationCurve;
    BOOL _dgc_hasNotification;
    
    CGRect _dgc_observedToFrame;
    BOOL _dgc_hasObservedChange;
    
    BOOL _dgc_lastIsNotification;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"DGCYYTextKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}

- (instancetype)_init {
    self = [super init];
    _dgc_observers = [[NSHashTable alloc] initWithOptions:NSPointerFunctionsWeakMemory|NSPointerFunctionsObjectPointerPersonality capacity:0];
...
- (void)_initFrameObserver {
    ...
    _DGCYYTextKeyboardViewFrameObserver *observer = [_DGCYYTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_DGCYYTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager {
    static DGCYYTextKeyboardManager *mgr = nil;
...
- (void)addObserver:(id<DGCYYTextKeyboardObserver>)observer {
    if (!observer) return;
    [_dgc_observers addObject:observer];
}

- (void)removeObserver:(id<DGCYYTextKeyboardObserver>)observer {
    if (!observer) return;
    [_dgc_observers removeObject:observer];
}
...
- (void)_keyboardFrameWillChangeNotification:(NSNotification *)notif {
    ...
    _dgc_notificationFromFrame = before;
    _dgc_notificationToFrame = after;
    _dgc_notificationCurve = curve;
    _dgc_notificationDuration = duration;
    _dgc_hasNotification = YES;
    _dgc_lastIsNotification = YES;
...
- (void)_keyboardFrameDidChangeNotification:(NSNotification *)notif {
    ...
    _dgc_notificationToFrame = after;
    _dgc_notificationCurve = UIViewAnimationCurveEaseInOut;
    _dgc_notificationDuration = 0;
    _dgc_hasNotification = YES;
    _dgc_lastIsNotification = YES;
...
- (void)_keyboardFrameChanged:(UIView *)keyboard {
    ...
    UIWindow *window = keyboard.window;
    if (window) {
        _dgc_observedToFrame = [window convertRect:keyboard.frame toWindow:nil];
    } else {
        _dgc_observedToFrame = keyboard.frame;
    }
    _dgc_hasObservedChange = YES;
    _dgc_lastIsNotification = NO;
...
- (void)_notifyAllObservers {
    ...
    DGCYYTextKeyboardTransition trans = {0};
    
    // from
    if (_dgc_fromFrame.size.width == 0 && _dgc_fromFrame.size.height == 0) { // first notify
        _dgc_fromFrame.size.width = window.bounds.size.width;
        _dgc_fromFrame.size.height = trans.toFrame.size.height;
        _dgc_fromFrame.origin.x = trans.toFrame.origin.x;
        _dgc_fromFrame.origin.y = window.bounds.size.height;
    }
    trans.fromFrame = _dgc_fromFrame;
    trans.fromVisible = _dgc_fromVisible;
    
    // to
    if (_dgc_lastIsNotification || (_dgc_hasObservedChange && CGRectEqualToRect(_dgc_observedToFrame, _dgc_notificationToFrame))) {
        trans.toFrame = _dgc_notificationToFrame;
        trans.animationDuration = _dgc_notificationDuration;
        trans.animationCurve = _dgc_notificationCurve;
        trans.animationOption = _dgc_notificationCurve << 16;
        ...
        if (((int)[self _systemVersion]) == 7) {
            ...
            if (_dgc_fromOrientation != UIInterfaceOrientationUnknown && _dgc_fromOrientation != ori) {
                switch (ori) {
        ...
        trans.toFrame = _dgc_observedToFrame;
    }
    ...
    if (!CGRectEqualToRect(trans.toFrame, _dgc_fromFrame)) {
        for (id<DGCYYTextKeyboardObserver> observer in _dgc_observers.copy) {
    ...
    _dgc_hasNotification = NO;
    _dgc_hasObservedChange = NO;
    _dgc_fromFrame = trans.toFrame;
    _dgc_fromVisible = trans.toVisible;
    _dgc_fromOrientation = app.statusBarOrientation;
}
