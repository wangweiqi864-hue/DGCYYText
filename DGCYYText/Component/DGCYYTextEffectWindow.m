...
NSString *const DGCYYTextViewWindowBecomeKeyWindowNotification = @"DGCYYTextViewWindowBecomeKeyWindowNotification";

@implementation DGCYYTextEffectWindow

+ (instancetype)sharedWindow {
    static DGCYYTextEffectWindow *one = nil;
    if (one == nil) {
        // iOS 9 compatible
    ...
    dispatch_once(&onceToken, ^{
        if (!DGCYYTextIsAppExtension()) {
            one = [self new];
            one.frame = (CGRect){.size = DGCYYTextScreenSize()};
...
- (void)becomeKeyWindow {
    id<UIApplicationDelegate> delegate = DGCYYTextSharedApplication().delegate;
    if (delegate == nil) {
        return;
    }
    ...
    if ([delegate respondsToSelector:@selector(window)]) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:DGCYYTextViewWindowBecomeKeyWindowNotification object:nil];
    }
}

- (UIViewController *)rootViewController {
    for (UIWindow *window in [DGCYYTextSharedApplication() windows]) {
...
- (DGCYYTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [DGCYYTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[DGCYYTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return DGCYYTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return DGCYYTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return DGCYYTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return DGCYYTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return DGCYYTextDirectionLeft;
    
    return DGCYYTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(DGCYYTextMagnifier *)mag rotation:(CGFloat)rotation {
    CGFloat degree = DGCYYTextRadiansToDegrees(rotation);
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == DGCYYTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == DGCYYTextMagnifierTypeRanged) {
 ...
 */
- (CGFloat)_updateMagnifier:(DGCYYTextMagnifier *)mag {
    UIApplication *app = DGCYYTextSharedApplication();
    if (!app) return 0;
    ...
    CGAffineTransform trans = DGCYYTextCGAffineTransformGetFromViews(hostView, self);
    CGFloat rotation = DGCYYTextCGAffineTransformGetRotation(trans);
    ...
    if (captureRect.size.width > 0 && captureRect.size.height > 0) {
        ...
        for (UIWindow *window in windows) {
            ...
            CGContextConcatCTM(context, DGCYYTextCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(DGCYYTextMagnifier *)mag {
    ...
    mag.transform = trans;
    mag.center = center;
    if (mag.type == DGCYYTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == DGCYYTextMagnifierTypeCaret ? 0.08 : 0.1;
...
- (void)hideMagnifier:(DGCYYTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self yy_convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == DGCYYTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(DGCYYSelectionGrabberDot *)dot selection:(DGCYYTextSelectionView *)selection{
    ...
    if (selection.hostView.clipsToBounds == YES && dot.yy_visibleAlpha > 0.1) {
        ...
        if (!dotInKeyboard) {
            ...
            if (DGCYYTextCGRectGetArea(intersection) < DGCYYTextCGRectGetArea(dotRect)) {
                CGFloat dist = DGCYYTextCGPointGetDistanceToRect(DGCYYTextCGRectGetCenter(dotRect), hostRect);
