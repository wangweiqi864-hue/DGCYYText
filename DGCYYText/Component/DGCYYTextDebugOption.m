...
static DGCYYTextDebugOption *_sharedDebugOption = nil;
...
void _sharedDebugSetFunction(const void *value, void *context) {
    id<DGCYYTextDebugTarget> target = (__bridge id<DGCYYTextDebugTarget>)(value);
...
static void _setSharedDebugOption(DGCYYTextDebugOption *option) {
    _initSharedDebug();
...
static DGCYYTextDebugOption *_getSharedDebugOption(void) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
    DGCYYTextDebugOption *op = _sharedDebugOption;
    pthread_mutex_unlock(&_sharedDebugLock);
    return op;
}

static void _addDebugTarget(id<DGCYYTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
static void _removeDebugTarget(id<DGCYYTextDebugTarget> target) {
    _initSharedDebug();
    pthread_mutex_lock(&_sharedDebugLock);
...
@implementation DGCYYTextDebugOption

- (id)copyWithZone:(NSZone *)zone {
    DGCYYTextDebugOption *op = [self.class new];
...
+ (void)addDebugTarget:(id<DGCYYTextDebugTarget>)target {
    if (target) _addDebugTarget(target);
}

+ (void)removeDebugTarget:(id<DGCYYTextDebugTarget>)target {
    if (target) _removeDebugTarget(target);
}

+ (DGCYYTextDebugOption *)sharedDebugOption {
    return _getSharedDebugOption();
}

+ (void)setSharedDebugOption:(DGCYYTextDebugOption *)option {
