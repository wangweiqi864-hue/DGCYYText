...
const CGSize DGCYYTextContainerMaxSize = (CGSize){0x100000, 0x100000};
...
static inline CGSize DGCYYTextClipCGSize(CGSize size) {
    if (size.width > DGCYYTextContainerMaxSize.width) size.width = DGCYYTextContainerMaxSize.width;
    if (size.height > DGCYYTextContainerMaxSize.height) size.height = DGCYYTextContainerMaxSize.height;
    return size;
}
 ...
 */
static CGColorRef DGCYYTextGetCGColor(CGColorRef color) {
...
@implementation DGCYYTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(DGCYYTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            DGCYYTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    DGCYYTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation DGCYYTextContainer {
    @package
    BOOL _dgc_readonly; ///< used only in DGCYYTextLayout.implementation
    dispatch_semaphore_t _dgc_lock;
    
    CGSize _dgc_size;
    UIEdgeInsets _dgc_insets;
    UIBezierPath *_dgc_path;
    NSArray *_dgc_exclusionPaths;
    BOOL _dgc_pathFillEvenOdd;
    CGFloat _dgc_pathLineWidth;
    BOOL _dgc_verticalForm;
    NSUInteger _dgc_maximumNumberOfRows;
    DGCYYTextTruncationType _dgc_truncationType;
    NSAttributedString *_dgc_truncationToken;
    id<DGCYYTextLinePositionModifier> _dgc_linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    DGCYYTextContainer *one = [self new];
    one.size = DGCYYTextClipCGSize(size);
    one.insets = insets;
    return one;
}
...
- (instancetype)init {
    self = [super init];
    if (!self) return nil;
    _dgc_lock = dispatch_semaphore_create(1);
    _dgc_pathFillEvenOdd = YES;
    return self;
}

- (id)copyWithZone:(NSZone *)zone {
    DGCYYTextContainer *one = [self.class new];
    dispatch_semaphore_wait(_dgc_lock, DISPATCH_TIME_FOREVER);
    one->_size = _dgc_size;
    one->_insets = _dgc_insets;
    one->_path = _dgc_path;
    one->_exclusionPaths = _dgc_exclusionPaths.copy;
    one->_pathFillEvenOdd = _dgc_pathFillEvenOdd;
    one->_pathLineWidth = _dgc_pathLineWidth;
    one->_verticalForm = _dgc_verticalForm;
    one->_maximumNumberOfRows = _dgc_maximumNumberOfRows;
    one->_truncationType = _dgc_truncationType;
    one->_truncationToken = _dgc_truncationToken.copy;
    one->_linePositionModifier = [(NSObject *)_dgc_linePositionModifier copy];
    dispatch_semaphore_signal(_dgc_lock);
    return one;
}
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    [aCoder encodeObject:[NSValue valueWithCGSize:_dgc_size] forKey:@"size"];
    [aCoder encodeObject:[NSValue valueWithUIEdgeInsets:_dgc_insets] forKey:@"insets"];
    [aCoder encodeObject:_dgc_path forKey:@"path"];
    [aCoder encodeObject:_dgc_exclusionPaths forKey:@"exclusionPaths"];
    [aCoder encodeBool:_dgc_pathFillEvenOdd forKey:@"pathFillEvenOdd"];
    [aCoder encodeDouble:_dgc_pathLineWidth forKey:@"pathLineWidth"];
    [aCoder encodeBool:_dgc_verticalForm forKey:@"verticalForm"];
    [aCoder encodeInteger:_dgc_maximumNumberOfRows forKey:@"maximumNumberOfRows"];
    [aCoder encodeInteger:_dgc_truncationType forKey:@"truncationType"];
    [aCoder encodeObject:_dgc_truncationToken forKey:@"truncationToken"];
    if ([_dgc_linePositionModifier respondsToSelector:@selector(encodeWithCoder:)] &&
        [_dgc_linePositionModifier respondsToSelector:@selector(initWithCoder:)]) {
        [aCoder encodeObject:_dgc_linePositionModifier forKey:@"linePositionModifier"];
    }
}

- (id)initWithCoder:(NSCoder *)aDecoder {
    self = [self init];
    _dgc_size = ((NSValue *)[aDecoder decodeObjectForKey:@"size"]).CGSizeValue;
    _dgc_insets = ((NSValue *)[aDecoder decodeObjectForKey:@"insets"]).UIEdgeInsetsValue;
    _dgc_path = [aDecoder decodeObjectForKey:@"path"];
    _dgc_exclusionPaths = [aDecoder decodeObjectForKey:@"exclusionPaths"];
    _dgc_pathFillEvenOdd = [aDecoder decodeBoolForKey:@"pathFillEvenOdd"];
    _dgc_pathLineWidth = [aDecoder decodeDoubleForKey:@"pathLineWidth"];
    _dgc_verticalForm = [aDecoder decodeBoolForKey:@"verticalForm"];
    _dgc_maximumNumberOfRows = [aDecoder decodeIntegerForKey:@"maximumNumberOfRows"];
    _dgc_truncationType = [aDecoder decodeIntegerForKey:@"truncationType"];
    _dgc_truncationToken = [aDecoder decodeObjectForKey:@"truncationToken"];
    _dgc_linePositionModifier = [aDecoder decodeObjectForKey:@"linePositionModifier"];
    return self;
}
...
reason:@"Cannot change the property of the 'container' in 'DGCYYTextLayout'." userInfo:nil]; \
return; \
} \
dispatch_semaphore_wait(_dgc_lock, DISPATCH_TIME_FOREVER); \
__VA_ARGS__; \
...
- (CGSize)size {
    Getter(CGSize size = _dgc_size) return size;
}

- (void)setSize:(CGSize)size {
    Setter(if(!_dgc_path) _dgc_size = DGCYYTextClipCGSize(size));
}

- (UIEdgeInsets)insets {
    Getter(UIEdgeInsets insets = _dgc_insets) return insets;
}
...
- (UIBezierPath *)path {
    Getter(UIBezierPath *path = _dgc_path) return path;
}
...
- (void)setPath:(UIBezierPath *)path {
    Setter(
           _dgc_path = path.copy;
           if (_dgc_path) {
               CGRect bounds = _dgc_path.bounds;
...
- (NSArray *)exclusionPaths {
    Getter(NSArray *paths = _dgc_exclusionPaths) return paths;
}

- (void)setExclusionPaths:(NSArray *)exclusionPaths {
    Setter(_dgc_exclusionPaths = exclusionPaths.copy);
}

- (BOOL)isPathFillEvenOdd {
    Getter(BOOL is = _dgc_pathFillEvenOdd) return is;
}

- (void)setPathFillEvenOdd:(BOOL)pathFillEvenOdd {
    Setter(_dgc_pathFillEvenOdd = pathFillEvenOdd);
}

- (CGFloat)pathLineWidth {
    Getter(CGFloat width = _dgc_pathLineWidth) return width;
}

- (void)setPathLineWidth:(CGFloat)pathLineWidth {
    Setter(_dgc_pathLineWidth = pathLineWidth);
}

- (BOOL)isVerticalForm {
    Getter(BOOL v = _dgc_verticalForm) return v;
}

- (void)setVerticalForm:(BOOL)verticalForm {
    Setter(_dgc_verticalForm = verticalForm);
}

- (NSUInteger)maximumNumberOfRows {
    Getter(NSUInteger num = _dgc_maximumNumberOfRows) return num;
}

- (void)setMaximumNumberOfRows:(NSUInteger)maximumNumberOfRows {
    Setter(_dgc_maximumNumberOfRows = maximumNumberOfRows);
}

- (DGCYYTextTruncationType)truncationType {
    Getter(DGCYYTextTruncationType type = _dgc_truncationType) return type;
}

- (void)setTruncationType:(DGCYYTextTruncationType)truncationType {
    Setter(_dgc_truncationType = truncationType);
}

- (NSAttributedString *)truncationToken {
    Getter(NSAttributedString *token = _dgc_truncationToken) return token;
}

- (void)setTruncationToken:(NSAttributedString *)truncationToken {
    Setter(_dgc_truncationToken = truncationToken.copy);
}

- (void)setLinePositionModifier:(id<DGCYYTextLinePositionModifier>)linePositionModifier {
    Setter(_dgc_linePositionModifier = [(NSObject *)linePositionModifier copy]);
}

- (id<DGCYYTextLinePositionModifier>)linePositionModifier {
    Getter(id<DGCYYTextLinePositionModifier> m = _dgc_linePositionModifier) return m;
}
...
@interface DGCYYTextLayout ()

@property (nonatomic, readwrite) DGCYYTextContainer *container;
...
@property (nonatomic, readwrite) DGCYYTextLine *truncatedLine;
...
@implementation DGCYYTextLayout
...
+ (DGCYYTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    DGCYYTextContainer *container = [DGCYYTextContainer containerWithSize:size];
    return [self layoutWithContainer:container text:text];
}

+ (DGCYYTextLayout *)layoutWithContainer:(DGCYYTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (DGCYYTextLayout *)layoutWithContainer:(DGCYYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    DGCYYTextLayout *layout = NULL;
    ...
    DGCYYTextLine *truncatedLine = nil;
    ...
    layout = [[DGCYYTextLayout alloc] _init];
    layout.text = text;
    ...
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        if (needFixLayoutSizeBug) {
            ...
            if (container.isVerticalForm) {
                rect.size.width = DGCYYTextContainerMaxSize.width;
            } else {
                rect.size.height = DGCYYTextContainerMaxSize.height;
            }
        }
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, DGCYYTextCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
...
//        // fix: 阿语环境下 位置计算错位问题 不知道为啥 lineOrigins 首次计算正确，第二次开始不正常 ，先强制设置为0
//        ctLineOrigin.x = 0;
        ...
        DGCYYTextLine *line = [DGCYYTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    //fix: 这里处理阿语环境
    UISemanticContentAttribute attribute = [UIView appearance].semanticContentAttribute;
...
//        NSLog(@"aaaaa00000----%@",NSStringFromCGRect(textBoundingRect));
        
        CGFloat mWidth = 0;
        
        NSMutableArray *nLines = [NSMutableArray new];
        for (int i = 0; i< lines.count; i++) {
            DGCYYTextLine *line = lines[i];
            CGRect rect = line.bounds;
            mWidth = MAX(rect.size.width, mWidth);
//            NSLog(@"aaaaa1111----%@",NSStringFromCGRect(line.bounds));
        }
        
        for (int i = 0; i< lines.count; i++) {
            DGCYYTextLine *line = lines[i];
            CGPoint position = line.position;
            position.x = line.bounds.origin.x + mWidth - line.bounds.size.width;
            DGCYYTextLine *nLine = [DGCYYTextLine lineWithCTLine:line.CTLine position:position vertical:line.vertical];
            [nLines addObject:nLine];
//            if (i == lines.count - 1) {
//                mHeight = nLine.bounds.origin.y + nLine.bounds.size.height - mY;
//            }
            
            CGRect rect = nLine.bounds;
            if (i == 0) textBoundingRect = rect;
            else {
                if (maximumNumberOfRows == 0 || rowIdx < maximumNumberOfRows) {
                    textBoundingRect = CGRectUnion(textBoundingRect, rect);
...
//            NSLog(@"aaaaa2222----%@",NSStringFromCGRect(nLine.bounds));
        }
//        if (UIEdgeInsetsEqualToEdgeInsets(container.insets, UIEdgeInsetsZero)) {
//            textBoundingRect = CGRectMake(0, mY, mWidth, mHeight);
//        }else{
//            textBoundingRect = CGRectMake(container.insets.left, mY, mWidth, mHeight);
//        }
        
//        NSLog(@"aaaaa3333----%@",NSStringFromCGRect(textBoundingRect));
        lines = nLines;
    }else{
        for (int i = 0; i< lines.count; i++) {
            DGCYYTextLine *line = lines[i];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    DGCYYTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        DGCYYTextLine *lastLine = lines.lastObject;
    ...
    { // calculate bounding size
        ...
        if (container.path) {
            ...
            rect = UIEdgeInsetsInsetRect(rect,DGCYYTextUIEdgeInsetsInvert(container.insets));
        }
        rect = CGRectStandardize(rect);
    ...
    visibleRange = DGCYYTextNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        DGCYYTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != DGCYYTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:DGCYYTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == DGCYYTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == DGCYYTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [DGCYYTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = DGCYYTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = DGCYYTextVerticalFormRotateAndMoveCharacterSet();
        
        void (^lineBlock)(DGCYYTextLine *) = ^(DGCYYTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                BOOL isColorGlyph = DGCYYTextCTFontContainsColorBitmapGlyphs(font);
                
                NSUInteger prevIdx = 0;
                DGCYYTextRunGlyphDrawMode prevMode = DGCYYTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    DGCYYTextRunGlyphDrawMode mode = glyphRotateMove ? DGCYYTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? DGCYYTextRunGlyphDrawModeVerticalRotate : DGCYYTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    } else if (mode != prevMode) {
                        DGCYYTextRunGlyphRange *aRange = [DGCYYTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    DGCYYTextRunGlyphRange *aRange = [DGCYYTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (DGCYYTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[DGCYYTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[DGCYYTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[DGCYYTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[DGCYYTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[DGCYYTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[DGCYYTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[DGCYYTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[DGCYYTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[DGCYYTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (DGCYYTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        DGCYYTextContainer *container = containers[i];
        DGCYYTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [DGCYYTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [DGCYYTextUnarchiver unarchiveObjectWithData:textData];
    DGCYYTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
 ...
 */
- (CTRunRef)_runForLine:(DGCYYTextLine *)line position:(DGCYYTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == DGCYYTextAffinityBackward) {
...
/**
 ...
 @param block    The block to be executed before returns YES.
            ...
            next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(DGCYYTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(DGCYYTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (!DGCYYTextCTFontContainsColorBitmapGlyphs(font)) continue;
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(DGCYYTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (DGCYYTextRange *)_correctedRangeWithEdge:(DGCYYTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    DGCYYTextPosition *start = range.start;
    DGCYYTextPosition *end = range.end;
    
    if (start.offset == visibleRange.location && start.affinity == DGCYYTextAffinityBackward) {
        start = [DGCYYTextPosition positionWithOffset:start.offset affinity:DGCYYTextAffinityForward];
    }
    
    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == DGCYYTextAffinityForward) {
        end = [DGCYYTextPosition positionWithOffset:end.offset affinity:DGCYYTextAffinityBackward];
    }
    
    if (start != range.start || end != range.end) {
        range = [DGCYYTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((DGCYYTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        if (i >= _lines.count) { // 越界了
            continue;
        }
        CGRect bounds = ((DGCYYTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    DGCYYTextLine *line = _lines[lineIndex];
...
- (NSUInteger)textPositionForPoint:(CGPoint)point lineIndex:(NSUInteger)lineIndex {
    ...
    for (NSUInteger r = 0, max = CFArrayGetCount(runs); r < max; r++) {
        ...
        if (range.location <= idx && idx < range.location + range.length) {
            ...
            CFDictionaryRef attrs = CTRunGetAttributes(run);
            CTFontRef font = CFDictionaryGetValue(attrs, kCTFontAttributeName);
            if (!DGCYYTextCTFontContainsColorBitmapGlyphs(font)) break;
...
- (DGCYYTextPosition *)closestPositionToPoint:(CGPoint)point {
    BOOL isVertical = _container.verticalForm;
    ...
    if (position <= _visibleRange.location) {
        return [DGCYYTextPosition positionWithOffset:_visibleRange.location affinity:DGCYYTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [DGCYYTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:DGCYYTextAffinityBackward];
    }
    
    DGCYYTextAffinity finalAffinity = DGCYYTextAffinityForward;
    BOOL finalAffinityDetected = NO;
    
    // binding range
    NSRange bindingRange;
    DGCYYTextBinding *binding = [_text attribute:DGCYYTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[DGCYYTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[DGCYYTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:DGCYYTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = DGCYYTextAffinityForward;
                    } else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = DGCYYTextAffinityBackward;
                    }
                } else {
    ...
    if (line.range.length == 0) {
        ...
        return [DGCYYTextPosition positionWithOffset:line.range.location affinity:behind ? DGCYYTextAffinityBackward:DGCYYTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (DGCYYTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [DGCYYTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (DGCYYTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (DGCYYTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [DGCYYTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [DGCYYTextPosition positionWithOffset:position affinity:DGCYYTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? DGCYYTextAffinityForward : DGCYYTextAffinityBackward;
            } else if (position <= line.range.location) {
                finalAffinity = RTL ? DGCYYTextAffinityBackward : DGCYYTextAffinityForward;
            } else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? DGCYYTextAffinityForward : DGCYYTextAffinityBackward;
            }
        }
    }
    
    return [DGCYYTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (DGCYYTextPosition *)positionForPoint:(CGPoint)point
                         oldPosition:(DGCYYTextPosition *)oldPosition
                       otherPosition:(DGCYYTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    DGCYYTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            DGCYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        } else { // search forward
            DGCYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    } else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            DGCYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        } else { // search forward
            DGCYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (DGCYYTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    DGCYYTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        DGCYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    } else {
        DGCYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (DGCYYTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    DGCYYTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    DGCYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (DGCYYTextRange *)textRangeByExtendingPosition:(DGCYYTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [DGCYYTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [DGCYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:DGCYYTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    DGCYYTextBinding *binding = [_text attribute:DGCYYTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [DGCYYTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [DGCYYTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [DGCYYTextRange rangeWithStart:[DGCYYTextPosition positionWithOffset:position.offset - 1] end:[DGCYYTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (DGCYYTextIsLinebreakChar(c0) && position.affinity == DGCYYTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = DGCYYTextLinebreakTailLength(str);
            return [DGCYYTextRange rangeWithStart:[DGCYYTextPosition positionWithOffset:position.offset - len] end:[DGCYYTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [DGCYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (DGCYYTextRange *)textRangeByExtendingPosition:(DGCYYTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [DGCYYTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [DGCYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:DGCYYTextAffinityBackward];
    }
    
    // extend from position
    DGCYYTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    DGCYYTextRange *allForward = [DGCYYTextRange rangeWithStart:fromRange.start end:[DGCYYTextPosition positionWithOffset:visibleEnd]];
    DGCYYTextRange *allBackward = [DGCYYTextRange rangeWithStart:[DGCYYTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        DGCYYTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        DGCYYTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        DGCYYTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = DGCYYTextLinebreakTailLength(subStr);
                extPos = [DGCYYTextPosition positionWithOffset:pos - lineBreakLen];
            } else {
                extPos = [DGCYYTextPosition positionWithOffset:pos];
            }
        } else {
            extPos = [DGCYYTextPosition positionWithOffset:pos];
        }
        DGCYYTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [DGCYYTextRange rangeWithStart:fromRange.start end:ext.end];
        } else {
            return [DGCYYTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    } else { // left/right in text layout
        DGCYYTextPosition *toPosition = [DGCYYTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        DGCYYTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [DGCYYTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}

- (NSUInteger)lineIndexForPosition:(DGCYYTextPosition *)position {
...
- (CGPoint)linePositionForPosition:(DGCYYTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)caretRectForPosition:(DGCYYTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(DGCYYTextRange *)range {
    ...
    DGCYYTextLine *startLine = _lines[startLineIndex];
    DGCYYTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        DGCYYTextLine *line = _lines[i];
...
- (CGRect)rectForRange:(DGCYYTextRange *)range {
    NSArray *rects = [self selectionRectsForRange:range];
    if (rects.count == 0) return CGRectNull;
    CGRect rectUnion = ((DGCYYTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        DGCYYTextSelectionRect *rect = rects[i];
        rectUnion = CGRectUnion(rectUnion, rect.rect);
    }
    return rectUnion;
}

- (NSArray *)selectionRectsForRange:(DGCYYTextRange *)range {
    range = [self _correctedRangeWithEdge:range];
    ...
    DGCYYTextSelectionRect *start = [DGCYYTextSelectionRect new];
    if (isVertical) {
    ...
    DGCYYTextSelectionRect *end = [DGCYYTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        ...
        DGCYYTextSelectionRect *rect = [DGCYYTextSelectionRect new];
        if (isVertical) {
        ...
        DGCYYTextSelectionRect *topRect = [DGCYYTextSelectionRect new];
        ...
        DGCYYTextSelectionRect *bottomRect = [DGCYYTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(DGCYYTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(DGCYYTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        DGCYYTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, DGCYYTextDecorationType) {
    DGCYYTextDecorationTypeUnderline     = 1 << 0,
    DGCYYTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, DGCYYTextBorderType) {
    DGCYYTextBorderTypeBackgound = 1 << 0,
    DGCYYTextBorderTypeNormal    = 1 << 1,
};

static CGRect DGCYYTextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
    if (isVertical) {
...
static void DGCYYTextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void DGCYYTextDrawRun(DGCYYTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
    
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(DGCYYTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        fillColor = DGCYYTextGetCGColor(fillColor);
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (DGCYYTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    DGCYYTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context); {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == DGCYYTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
                                CGContextSetTextPosition(context, x, y);
                            } else {
                                CGContextRotateCTM(context, DGCYYTextDegreesToRadians(-90));
                                CGContextSetTextPosition(context,
                                                         line.position.y - size.height + glyphPositions[g].x,
                                                         line.position.x + verticalOffset + glyphPositions[g].y);
                            }
                            
                            if (DGCYYTextCTFontContainsColorBitmapGlyphs(runFont)) {
...
static void DGCYYTextSetLinePatternInContext(DGCYYTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == DGCYYTextLineStylePatternCircleDot) {
...
static void DGCYYTextDrawBorderRects(CGContextRef context, CGSize size, DGCYYTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;
    
    DGCYYTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    for (NSValue *value in rects) {
        ...
        rect = DGCYYTextCGRectPixelRound(rect);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        DGCYYTextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
        CGFloat inset = -border.strokeWidth * 0.5;
        if ((border.lineStyle & 0xFF) == DGCYYTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == DGCYYTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void DGCYYTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, DGCYYTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            CGFloat x, y1, y2, w;
            y1 = DGCYYTextCGFloatPixelRound(position.y);
            y2 = DGCYYTextCGFloatPixelRound(position.y + length);
            w = (styleBase == DGCYYTextLineStyleThick ? lineWidth * 2 : lineWidth);
            
            CGFloat linePixel = DGCYYTextCGFloatToPixel(w);
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    x = DGCYYTextCGFloatPixelHalf(position.x);
                } else {
                    x = DGCYYTextCGFloatPixelFloor(position.x);
                }
            } else {
                x = position.x;
            }
            ...
            DGCYYTextSetLinePatternInContext(style, lineWidth, position.y, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == DGCYYTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == DGCYYTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == DGCYYTextLineStyleDouble) {
            ...
            CGFloat x1, x2, y, w;
            x1 = DGCYYTextCGFloatPixelRound(position.x);
            x2 = DGCYYTextCGFloatPixelRound(position.x + length);
            w = (styleBase == DGCYYTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    y = DGCYYTextCGFloatPixelHalf(position.y);
                } else {
                    y = DGCYYTextCGFloatPixelFloor(position.y);
                }
            } else {
                y = position.y;
            }
            ...
            CGContextSetStrokeColorWithColor(context, color);
            DGCYYTextSetLinePatternInContext(style, lineWidth, position.x, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == DGCYYTextLineStyleSingle) {
...
static void DGCYYTextDrawText(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                DGCYYTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
            }
...
static void DGCYYTextDrawBlockBorder(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            DGCYYTextBorder *border = attrs[DGCYYTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((DGCYYTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((DGCYYTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                DGCYYTextLine *one = lines[lineContinueIndex];
                ...
                DGCYYTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    DGCYYTextBorder *nextBorder = [layout.text yy_attribute:DGCYYTextBlockBorderAttributeName atIndex:next.range.location];
            ...
            DGCYYTextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void DGCYYTextDrawBorder(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, DGCYYTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == DGCYYTextBorderTypeNormal ? DGCYYTextBorderAttributeName : DGCYYTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            DGCYYTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    DGCYYTextBorder *iBorder = iAttrs[borderKey];
            ...
            for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
                ...
                if (isVertical) {
                    if (fabs(rect.origin.x - curRect.origin.x) < 1) {
                        curRect = DGCYYTextMergeRectInSameLine(rect, curRect, isVertical);
                    } else {
            ...
            DGCYYTextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void DGCYYTextDrawDecoration(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, DGCYYTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            DGCYYTextDecoration *underline = attrs[DGCYYTextUnderlineAttributeName];
            DGCYYTextDecoration *strikethrough = attrs[DGCYYTextStrikethroughAttributeName];
            
            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & DGCYYTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & DGCYYTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (DGCYYTextIsLinebreakString(runStr)) continue; // may need more checks...
            
            CGFloat xHeight, underlinePosition, lineThickness;
            DGCYYTextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
                ...
                while (shadow) {
                    ...
                    CGContextSaveGState(context); {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            DGCYYTextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
                        } CGContextRestoreGState(context);
                    } CGContextRestoreGState(context);
                    shadow = shadow.subShadow;
                }
            ...
            if (needDrawStrikethrough) {
                ...
                DGCYYTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
            }
        }
    }
    CGContextRestoreGState(context);
...
static void DGCYYTextDrawAttachment(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        DGCYYTextAttachment *a = layout.attachments[i];
        ...
        rect = DGCYYTextCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = DGCYYTextCGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void DGCYYTextDrawShadow(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                DGCYYTextShadow *shadow = attrs[DGCYYTextShadowAttributeName];
                DGCYYTextShadow *nsShadow = [DGCYYTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void DGCYYTextDrawInnerShadow(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            DGCYYTextShadow *shadow = attrs[DGCYYTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void DGCYYTextDrawDebug(DGCYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, DGCYYTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / DGCYYTextScreenScale());
    ...
    if (op.CTFrameBorderColor || op.CTFrameFillColor) {
        ...
        if (!path) {
            ...
            if (op.CTFrameBorderColor) rect = DGCYYTextCGRectPixelHalf(rect);
            else rect = DGCYYTextCGRectPixelRound(rect);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTLineFillColor) {
            ...
            CGContextAddRect(context, DGCYYTextCGRectPixelRound(lineBounds));
            CGContextFillPath(context);
        }
        if (op.CTLineBorderColor) {
            [op.CTLineBorderColor setStroke];
            CGContextAddRect(context, DGCYYTextCGRectPixelHalf(lineBounds));
            CGContextStrokePath(context);
        }
        if (op.baselineColor) {
            [op.baselineColor setStroke];
            if (isVertical) {
                CGFloat x = DGCYYTextCGFloatPixelHalf(line.position.x);
                CGFloat y1 = DGCYYTextCGFloatPixelHalf(line.top);
                CGFloat y2 = DGCYYTextCGFloatPixelHalf(line.bottom);
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else {
                CGFloat x1 = DGCYYTextCGFloatPixelHalf(lineBounds.origin.x);
                CGFloat x2 = DGCYYTextCGFloatPixelHalf(lineBounds.origin.x + lineBounds.size.width);
                CGFloat y = DGCYYTextCGFloatPixelHalf(line.position.y);
                CGContextMoveToPoint(context, x1, y);
                CGContextAddLineToPoint(context, x2, y);
                CGContextStrokePath(context);
            }
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (op.CTRunFillColor) {
                    ...
                    CGContextAddRect(context, DGCYYTextCGRectPixelRound(runTypoBounds));
                    CGContextFillPath(context);
                }
                if (op.CTRunBorderColor) {
                    [op.CTRunBorderColor setStroke];
                    CGContextAddRect(context, DGCYYTextCGRectPixelHalf(runTypoBounds));
                    CGContextStrokePath(context);
                }
                ...
                if (op.CGGlyphBorderColor || op.CGGlyphFillColor) {
                    for (NSUInteger g = 0; g < glyphCount; g++) {
                        ...
                        if (op.CGGlyphFillColor) {
                            ...
                            CGContextAddRect(context, DGCYYTextCGRectPixelRound(rect));
                            CGContextFillPath(context);
                        }
                        if (op.CGGlyphBorderColor) {
                            [op.CGGlyphBorderColor setStroke];
                            CGContextAddRect(context, DGCYYTextCGRectPixelHalf(rect));
                            CGContextStrokePath(context);
                        }
                    }
                }
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(DGCYYTextDebugOption *)debug
                cancel:(BOOL (^)(void))cancel{
    @autoreleasepool {
        if (self.needDrawBlockBorder && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawBlockBorder(self, context, size, point, cancel);
        }
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawBorder(self, context, size, point, DGCYYTextBorderTypeBackgound, cancel);
        }
        if (self.needDrawShadow && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawShadow(self, context, size, point, cancel);
        }
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawDecoration(self, context, size, point, DGCYYTextDecorationTypeUnderline, cancel);
        }
        if (self.needDrawText && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawText(self, context, size, point, cancel);
        }
        if (self.needDrawAttachment && (context || view || layer)) {
            if (cancel && cancel()) return;
            DGCYYTextDrawAttachment(self, context, size, point, view, layer, cancel);
        }
        if (self.needDrawInnerShadow && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawInnerShadow(self, context, size, point, cancel);
        }
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawDecoration(self, context, size, point, DGCYYTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawBorder(self, context, size, point, DGCYYTextBorderTypeNormal, cancel);
        }
        if (debug.needDrawDebug && context) {
            if (cancel && cancel()) return;
            DGCYYTextDrawDebug(self, context, size, point, debug);
        }
    }
}
...
- (void)removeAttachmentFromViewAndLayer {
    NSAssert([NSThread isMainThread], @"This method must be called on the main thread");
    for (DGCYYTextAttachment *a in self.attachments) {
