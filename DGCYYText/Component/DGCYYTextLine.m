...
@implementation DGCYYTextLine {
    CGFloat _dgc_firstGlyphPos; // first glyph position for baseline, typically 0.
}

+ (instancetype)lineWithCTLine:(CTLineRef)CTLine position:(CGPoint)position vertical:(BOOL)isVertical {
    if (!CTLine) return nil;
    DGCYYTextLine *line = [self new];
    ...
    if (_CTLine != CTLine) {
        ...
        if (_CTLine) {
            ...
            if (CTLineGetGlyphCount(_CTLine) > 0) {
                ...
                _dgc_firstGlyphPos = pos.x;
            } else {
                _dgc_firstGlyphPos = 0;
            }
            _trailingWhitespaceWidth = CTLineGetTrailingWhitespaceWidth(_CTLine);
        } else {
            _lineWidth = _ascent = _descent = _leading = _dgc_firstGlyphPos = _trailingWhitespaceWidth = 0;
...
- (void)reloadBounds {
    if (_vertical) {
        ...
        _bounds.origin.y += _dgc_firstGlyphPos;
    } else {
        _bounds = CGRectMake(_position.x, _position.y - _ascent, _lineWidth, _ascent + _descent);
        _bounds.origin.x += _dgc_firstGlyphPos;
    }
    ...
    for (NSUInteger r = 0; r < runCount; r++) {
        ...
        DGCYYTextAttachment *attachment = attrs[DGCYYTextAttachmentAttributeName];
        if (attachment) {
            ...
            NSRange runRange = DGCYYTextNSRangeFromCFRange(CTRunGetStringRange(run));
...
- (NSString *)description {
    ...
    [desc appendFormat:@"<DGCYYTextLine: %p> row:%zd range:%tu,%tu",self, self.row, range.location, range.length];
