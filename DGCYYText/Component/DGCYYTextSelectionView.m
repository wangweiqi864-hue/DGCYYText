...
@implementation DGCYYSelectionGrabberDot
...
@implementation DGCYYSelectionGrabber

- (instancetype) initWithFrame:(CGRect)frame {
    self = [super initWithFrame:frame];
    if (!self) return nil;
    _dot = [[DGCYYSelectionGrabberDot alloc] initWithFrame:CGRectMake(0, 0, 10, 10)];
    return self;
}

- (void)setDotDirection:(DGCYYTextDirection)dotDirection {
    _dotDirection = dotDirection;
    [self addSubview:_dot];
    CGRect frame = _dot.frame;
    CGFloat ofs = 0.5;
    if (dotDirection == DGCYYTextDirectionTop) {
        frame.origin.y = -frame.size.height + ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == DGCYYTextDirectionRight) {
        frame.origin.x = self.bounds.size.width - ofs;
        frame.origin.y = (self.bounds.size.height - frame.size.height) / 2;
    } else if (dotDirection == DGCYYTextDirectionBottom) {
        frame.origin.y = self.bounds.size.height - ofs;
        frame.origin.x = (self.bounds.size.width - frame.size.width) / 2;
    } else if (dotDirection == DGCYYTextDirectionLeft) {
...
- (CGRect)touchRect {
    ...
    if (_dotDirection == DGCYYTextDirectionTop) {
        insets.top = -kTouchDotExtend;
    } else if (_dotDirection == DGCYYTextDirectionRight) {
        insets.right = -kTouchDotExtend;
    } else if (_dotDirection == DGCYYTextDirectionBottom) {
        insets.bottom = -kTouchDotExtend;
    } else if (_dotDirection == DGCYYTextDirectionLeft) {
...
@interface DGCYYTextSelectionView ()
@property (nonatomic, strong) NSTimer *caretTimer;
@property (nonatomic, strong) UIView *caretView;
@property (nonatomic, strong) DGCYYSelectionGrabber *startGrabber;
@property (nonatomic, strong) DGCYYSelectionGrabber *endGrabber;
@property (nonatomic, strong) NSMutableArray *markViews;
@end

@implementation DGCYYTextSelectionView
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    _startGrabber = [DGCYYSelectionGrabber new];
    _startGrabber.dotDirection = DGCYYTextDirectionTop;
    _startGrabber.hidden = YES;
    _endGrabber = [DGCYYSelectionGrabber new];
    _endGrabber.dotDirection = DGCYYTextDirectionBottom;
...
- (void)_startBlinks {
    [_caretTimer invalidate];
    if (_caretVisible) {
        _caretTimer = [NSTimer timerWithTimeInterval:kBlinkDuration target:[DGCYYTextWeakProxy proxyWithTarget:self] selector:@selector(_doBlink) userInfo:nil repeats:YES];
...
- (void)_doBlink {
    [UIView animateWithDuration:kBlinkFadeDuration delay:0 options:UIViewAnimationOptionCurveEaseInOut animations: ^{
        if (self->_caretView.alpha == 1) {
            self->_caretView.alpha = 0;
        } else {
            self->_caretView.alpha = 1;
        }
    } completion:NULL];
}
...
- (void)setVerticalForm:(BOOL)verticalForm {
    if (_verticalForm != verticalForm) {
        ...
        self.startGrabber.dotDirection = verticalForm ? DGCYYTextDirectionRight : DGCYYTextDirectionTop;
        self.endGrabber.dotDirection = verticalForm ? DGCYYTextDirectionLeft : DGCYYTextDirectionBottom;
    }
}
...
- (CGRect)_standardCaretRect:(CGRect)caretRect {
    ...
    caretRect = DGCYYTextCGRectPixelRound(caretRect);
...
- (void)setSelectionRects:(NSArray *)selectionRects {
    ...
    [selectionRects enumerateObjectsUsingBlock: ^(DGCYYTextSelectionRect *r, NSUInteger idx, BOOL *stop) {
        CGRect rect = r.rect;
...
- (BOOL)isStartGrabberContainsPoint:(CGPoint)point {
    ...
    if (CGRectIntersectsRect(startRect, endRect)) {
        CGFloat distStart = DGCYYTextCGPointGetDistanceToPoint(point, DGCYYTextCGRectGetCenter(startRect));
        CGFloat distEnd = DGCYYTextCGPointGetDistanceToPoint(point, DGCYYTextCGRectGetCenter(endRect));
...
- (BOOL)isSelectionRectsContainsPoint:(CGPoint)point {
    ...
    for (DGCYYTextSelectionRect *rect in _selectionRects) {
