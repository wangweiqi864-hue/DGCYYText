...
@implementation DGCYYTextArchiver

+ (NSData *)archivedDataWithRootObject:(id)rootObject {
    if (!rootObject) return nil;
    NSMutableData *data = [NSMutableData data];
    DGCYYTextArchiver *archiver = [[[self class] alloc] initForWritingWithMutableData:data];
...
- (id)archiver:(NSKeyedArchiver *)archiver willEncodeObject:(id)object {
    ...
    if (typeID == CTRunDelegateTypeID()) {
        ...
        DGCYYTextRubyAnnotation *ruby = [DGCYYTextRubyAnnotation rubyWithCTRubyRef:ctRuby];
        if (ruby) return ruby;
...
@implementation DGCYYTextUnarchiver

+ (id)unarchiveObjectWithData:(NSData *)data {
    if (data.length == 0) return nil;
    DGCYYTextUnarchiver *unarchiver = [[self alloc] initForReadingWithData:data];
...
- (id)unarchiver:(NSKeyedUnarchiver *)unarchiver didDecodeObject:(id) NS_RELEASES_ARGUMENT object NS_RETURNS_RETAINED {
    if ([object class] == [DGCYYTextRunDelegate class]) {
        DGCYYTextRunDelegate *runDelegate = object;
        CTRunDelegateRef ct = runDelegate.CTRunDelegate;
        id ctObj = (__bridge id)ct;
        if (ct) CFRelease(ct);
        return ctObj;
    } else if ([object class] == [DGCYYTextRubyAnnotation class]) {
        DGCYYTextRubyAnnotation *ruby = object;
