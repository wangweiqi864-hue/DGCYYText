...
@interface DGCYYTextEditExample () <DGCYYTextViewDelegate, DGCYYTextKeyboardObserver>
...
@implementation DGCYYTextEditExample
...
- (void)viewDidLoad {
    ...
    _debugSwitch.on = [DGCYYTextExampleHelper isDebug];
    ...
    [_debugSwitch addBlockForControlEvents:UIControlEventValueChanged block:^(UISwitch *switcher) {
        [DGCYYTextExampleHelper setDebug:switcher.isOn];
    }];
...
- (void)initImageView {
    ...
    UIImage *image = [[DGCYYImage alloc] initWithData:data scale:2];
    UIImageView *imageView = [[DGCYYAnimatedImageView alloc] initWithImage:image];
...
- (void)keyboardChangedWithTransition:(DGCYYTextKeyboardTransition)transition {
    ...
    if (_textView.isVerticalForm && transition.toVisible) {
        CGRect rect = [[DGCYYTextKeyboardManager defaultManager] convertRect:transition.toFrame toView:self.view];
