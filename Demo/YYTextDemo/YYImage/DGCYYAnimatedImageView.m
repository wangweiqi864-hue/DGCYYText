...
@interface DGCYYAnimatedImageView() {
    @package
    UIImage <YYAnimatedImage> *_dgc_curAnimatedImage;
    
    dispatch_once_t _dgc_onceToken;
    dispatch_semaphore_t _dgc_lock; ///< lock for _dgc_buffer
    NSOperationQueue *_dgc_requestQueue; ///< image request queue, serial
    
    CADisplayLink *_dgc_link; ///< ticker for change frame
    NSTimeInterval _dgc_time; ///< time after last frame
    
    UIImage *_dgc_curFrame; ///< current frame to display
    NSUInteger _dgc_curIndex; ///< current frame index (from 0)
    NSUInteger _dgc_totalFrameCount; ///< total frame count
    
    BOOL _dgc_loopEnd; ///< whether the loop is end.
    NSUInteger _dgc_curLoop; ///< current loop count (from 0)
    NSUInteger _dgc_totalLoop; ///< total loop count, 0 means infinity
    
    NSMutableDictionary *_dgc_buffer; ///< frame buffer
    BOOL _dgc_bufferMiss; ///< whether miss frame on last opportunity
    NSUInteger _dgc_maxBufferCount; ///< maximum buffer count
    NSInteger _dgc_incrBufferCount; ///< current allowed buffer count (will increase by step)
    
    CGRect _dgc_curContentsRect;
    BOOL _dgc_curImageHasContentsRect; ///< image has implementated "animatedImageContentsRectAtIndex:"
}
...
@property (nonatomic, weak) DGCYYAnimatedImageView *view;
...
- (void)main {
    __strong DGCYYAnimatedImageView *view = _dgc_view;
    if (!view) return;
...
@implementation DGCYYAnimatedImageView
...
- (void)resetAnimated {
    dispatch_once(&_dgc_onceToken, ^{
        _dgc_lock = dispatch_semaphore_create(1);
        _dgc_buffer = [NSMutableDictionary new];
        _dgc_requestQueue = [[NSOperationQueue alloc] init];
        _dgc_requestQueue.maxConcurrentOperationCount = 1;
        _dgc_link = [CADisplayLink displayLinkWithTarget:[_YYImageWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
            [_dgc_link addToRunLoop:[NSRunLoop mainRunLoop] forMode:_runloopMode];
        }
        _dgc_link.paused = YES;
        
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didReceiveMemoryWarning:) name:UIApplicationDidReceiveMemoryWarningNotification object:nil];
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(didEnterBackground:) name:UIApplicationDidEnterBackgroundNotification object:nil];
    });
    
    [_dgc_requestQueue cancelAllOperations];
    LOCK(
         if (_dgc_buffer.count) {
             NSMutableDictionary *holder = _dgc_buffer;
             _dgc_buffer = [NSMutableDictionary new];
    ...
    _dgc_curAnimatedImage = nil;
    _dgc_curFrame = nil;
    _dgc_curLoop = 0;
    _dgc_totalLoop = 0;
    _dgc_totalFrameCount = 1;
    _dgc_loopEnd = NO;
    _dgc_bufferMiss = NO;
    _dgc_incrBufferCount = 0;
}
...
- (void)setHighlighted:(BOOL)highlighted {
    ...
    if (_dgc_link) [self resetAnimated];
    [self imageChanged];
}
...
- (void)imageChanged {
    ...
    if (!hasContentsRect && _dgc_curImageHasContentsRect) {
    ...
    _dgc_curImageHasContentsRect = hasContentsRect;
    if (hasContentsRect) {
    ...
    if (newImageFrameCount > 1) {
        [self resetAnimated];
        _dgc_curAnimatedImage = newVisibleImage;
        _dgc_curFrame = newVisibleImage;
        _dgc_totalLoop = _dgc_curAnimatedImage.animatedImageLoopCount;
        _dgc_totalFrameCount = _dgc_curAnimatedImage.animatedImageFrameCount;
...
- (void)calcMaxBufferCount {
    int64_t bytes = (int64_t)_dgc_curAnimatedImage.animatedImageBytesPerFrame;
    ...
    _dgc_maxBufferCount = maxBufferCount;
}
...
- (void)startAnimating {
    ...
    if (type == YYAnimatedImageTypeImages || type == YYAnimatedImageTypeHighlightedImages) {
        ...
        if (_dgc_curAnimatedImage && _dgc_link.paused) {
            _dgc_curLoop = 0;
            _dgc_loopEnd = NO;
            _dgc_link.paused = NO;
            self.currentIsPlayingAnimation = YES;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification {
    [_dgc_requestQueue cancelAllOperations];
    [_dgc_requestQueue addOperationWithBlock: ^{
...
- (void)didEnterBackground:(NSNotification *)notification {
    [_dgc_requestQueue cancelAllOperations];
    NSNumber *next = @((_dgc_curIndex + 1) % _dgc_totalFrameCount);
    LOCK(
         NSArray * keys = _dgc_buffer.allKeys;
         for (NSNumber * key in keys) {
             if (![key isEqualToNumber:next]) { // keep the next frame for smoothly animation
                 [_dgc_buffer removeObjectForKey:key];
             }
         }
     )//LOCK
}
...
- (void)step:(CADisplayLink *)link {
    UIImage <YYAnimatedImage> *image = _dgc_curAnimatedImage;
    NSMutableDictionary *buffer = _dgc_buffer;
    UIImage *bufferedImage = nil;
    NSUInteger nextIndex = (_dgc_curIndex + 1) % _dgc_totalFrameCount;
    BOOL bufferIsFull = NO;
    
    if (!image) return;
    if (_dgc_loopEnd) { // view will keep in last frame
        [self stopAnimating];
        return;
    }
    ...
    if (!_dgc_bufferMiss) {
        _dgc_time += link.duration;
        delay = [image animatedImageDurationAtIndex:_dgc_curIndex];
        if (_dgc_time < delay) return;
        _dgc_time -= delay;
        if (nextIndex == 0) {
            _dgc_curLoop++;
            if (_dgc_curLoop >= _dgc_totalLoop && _dgc_totalLoop != 0) {
                _dgc_loopEnd = YES;
                [self stopAnimating];
        ...
        if (_dgc_time > delay) _dgc_time = delay; // do not jump over frame
    }
    LOCK(
         bufferedImage = buffer[@(nextIndex)];
         if (bufferedImage) {
             if ((int)_dgc_incrBufferCount < _dgc_totalFrameCount) {
                 [buffer removeObjectForKey:@(nextIndex)];
             }
             [self willChangeValueForKey:@"currentAnimatedImageIndex"];
             _dgc_curIndex = nextIndex;
             [self didChangeValueForKey:@"currentAnimatedImageIndex"];
             _dgc_curFrame = bufferedImage == (id)[NSNull null] ? nil : bufferedImage;
             if (_dgc_curImageHasContentsRect) {
                 _dgc_curContentsRect = [image animatedImageContentsRectAtIndex:_dgc_curIndex];
                 [self setContentsRect:_dgc_curContentsRect forImage:_dgc_curFrame];
             }
             nextIndex = (_dgc_curIndex + 1) % _dgc_totalFrameCount;
             _dgc_bufferMiss = NO;
             if (buffer.count == _dgc_totalFrameCount) {
                 bufferIsFull = YES;
             }
         } else {
             _dgc_bufferMiss = YES;
    ...
    if (!bufferIsFull && _dgc_requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _YYAnimatedImageViewFetchOperation *operation = [_YYAnimatedImageViewFetchOperation new];
        operation.view = self;
        operation.nextIndex = nextIndex;
        operation.curImage = image;
        [_dgc_requestQueue addOperation:operation];
    }
}

- (void)displayLayer:(CALayer *)layer {
    if (_dgc_curFrame) {
        layer.contents = (__bridge id)_dgc_curFrame.CGImage;
    }
}
...
- (void)setCurrentAnimatedImageIndex:(NSUInteger)currentAnimatedImageIndex {
    if (!_dgc_curAnimatedImage) return;
    if (currentAnimatedImageIndex >= _dgc_curAnimatedImage.animatedImageFrameCount) return;
    if (_dgc_curIndex == currentAnimatedImageIndex) return;
...
- (void)setRunloopMode:(NSString *)runloopMode {
    ...
    if (_dgc_link) {
        if (_runloopMode) {
            [_dgc_link removeFromRunLoop:[NSRunLoop mainRunLoop] forMode:_runloopMode];
        }
        if (runloopMode.length) {
            [_dgc_link addToRunLoop:[NSRunLoop mainRunLoop] forMode:runloopMode];
        }
    }
