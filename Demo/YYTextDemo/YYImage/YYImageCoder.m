...
@implementation DGCYYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    DGCYYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    DGCYYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _YYImageDecoderFrame : DGCYYImageFrame
...
@implementation DGCYYImageDecoder {
    pthread_mutex_t _dgc_lock; // recursive lock
    
    BOOL _dgc_sourceTypeDetected;
    CGImageSourceRef _dgc_source;
    yy_png_info *_dgc_apngSource;
#if YYIMAGE_WEBP_ENABLED
    WebPDemuxer *_dgc_webpSource;
#endif
    
    UIImageOrientation _dgc_orientation;
    dispatch_semaphore_t _dgc_framesLock;
    NSArray *_dgc_frames; ///< Array<GGImageDecoderFrame>, without image
    BOOL _dgc_needBlend;
    NSUInteger _dgc_blendFrameIndex;
    CGContextRef _dgc_blendCanvas;
}

- (void)dealloc {
    if (_dgc_source) CFRelease(_dgc_source);
    if (_dgc_apngSource) yy_png_info_release(_dgc_apngSource);
#if YYIMAGE_WEBP_ENABLED
    if (_dgc_webpSource) WebPDemuxDelete(_dgc_webpSource);
#endif
    if (_dgc_blendCanvas) CFRelease(_dgc_blendCanvas);
}

+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    DGCYYImageDecoder *decoder = [[DGCYYImageDecoder alloc] initWithScale:scale];
...
- (instancetype)initWithScale:(CGFloat)scale {
    ...
    if (scale <= 0) scale = 1;
    _scale = scale;
    _dgc_framesLock = dispatch_semaphore_create(1);
    
    pthread_mutexattr_t attr;
    pthread_mutexattr_init (&attr);
    pthread_mutexattr_settype (&attr, PTHREAD_MUTEX_RECURSIVE);
    pthread_mutex_init (&_dgc_lock, &attr);
...
- (BOOL)updateData:(NSData *)data final:(BOOL)final {
    BOOL result = NO;
    pthread_mutex_lock(&_dgc_lock);
    result = [self _updateData:data final:final];
    pthread_mutex_unlock(&_dgc_lock);
    return result;
}

- (DGCYYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    DGCYYImageFrame *result = nil;
    pthread_mutex_lock(&_dgc_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    dispatch_semaphore_wait(_dgc_framesLock, DISPATCH_TIME_FOREVER);
    if (index < _dgc_frames.count) {
        result = ((_YYImageDecoderFrame *)_dgc_frames[index]).duration;
    }
    dispatch_semaphore_signal(_dgc_framesLock);
    return result;
}
...
- (DGCYYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _dgc_frames.count) return 0;
    _YYImageDecoderFrame *frame = [(_YYImageDecoderFrame *)_dgc_frames[index] copy];
    BOOL decoded = NO;
    ...
    if (!_dgc_needBlend) {
        ...
        UIImage *image = [UIImage imageWithCGImage:imageRef scale:_scale orientation:_dgc_orientation];
        CFRelease(imageRef);
        if (!image) return nil;
    ...
    if (_dgc_blendFrameIndex + 1 == frame.index) {
        imageRef = [self _newBlendedImageWithFrame:frame];
        _dgc_blendFrameIndex = index;
    } else { // should draw canvas from previous frame
        _dgc_blendFrameIndex = NSNotFound;
        CGContextClearRect(_dgc_blendCanvas, CGRectMake(0, 0, _width, _height));
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (unblendedImage) {
                CGContextDrawImage(_dgc_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendedImage);
                CFRelease(unblendedImage);
            }
            imageRef = CGBitmapContextCreateImage(_dgc_blendCanvas);
            if (frame.dispose == YYImageDisposeBackground) {
                CGContextClearRect(_dgc_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
            }
            _dgc_blendFrameIndex = index;
            ...
            for (uint32_t i = (uint32_t)frame.blendFromIndex; i <= (uint32_t)frame.index; i++) {
                if (i == frame.index) {
                    ...
                    [self _blendImageWithFrame:_dgc_frames[i]];
                }
            }
            _dgc_blendFrameIndex = index;
        }
...
- (NSDictionary *)_framePropertiesAtIndex:(NSUInteger)index {
    if (index >= _dgc_frames.count) return nil;
    if (!_dgc_source) return nil;
    CFDictionaryRef properties = CGImageSourceCopyPropertiesAtIndex(_dgc_source, index, NULL);
...
- (NSDictionary *)_imageProperties {
    if (!_dgc_source) return nil;
    CFDictionaryRef properties = CGImageSourceCopyProperties(_dgc_source, NULL);
    if (!properties) return nil;
    return CFBridgingRelease(properties);
}
...
- (void)_updateSourceWebP {
    ...
    _dgc_needBlend = needBlend;
    _dgc_webpSource = demuxer;
    dispatch_semaphore_wait(_dgc_framesLock, DISPATCH_TIME_FOREVER);
    _dgc_frames = frames;
    dispatch_semaphore_signal(_dgc_framesLock);
#endif
...
- (void)_updateSourceAPNG {
    ...
    yy_png_info_release(_dgc_apngSource);
    _dgc_apngSource = nil;
    ...
    if (_dgc_source) { // apng decode succeed, no longer need image souce
        CFRelease(_dgc_source);
        _dgc_source = NULL;
    }
...
- (void)_updateSourceImageIO {
    _width = 0;
    _height = 0;
    _dgc_orientation = UIImageOrientationUp;
    _loopCount = 0;
    dispatch_semaphore_wait(_dgc_framesLock, DISPATCH_TIME_FOREVER);
    _dgc_frames = nil;
    dispatch_semaphore_signal(_dgc_framesLock);
    ...
    if (!_dgc_source) {
        if (_finalized) {
            _dgc_source = CGImageSourceCreateWithData((__bridge CFDataRef)_data, NULL);
        } else {
            _dgc_source = CGImageSourceCreateIncremental(NULL);
            if (_dgc_source) CGImageSourceUpdateData(_dgc_source, (__bridge CFDataRef)_data, false);
        }
    } else {
        CGImageSourceUpdateData(_dgc_source, (__bridge CFDataRef)_data, _finalized);
    }
    if (!_dgc_source) return;
    
    _frameCount = CGImageSourceGetCount(_dgc_source);
    ...
    for (NSUInteger i = 0; i < _frameCount; i++) {
        ...
        CFDictionaryRef properties = CGImageSourceCopyPropertiesAtIndex(_dgc_source, i, NULL);
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _dgc_orientation = YYUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    if (_dgc_frames.count <= index) return NULL;
    _YYImageDecoderFrame *frame = _dgc_frames[index];
    
    if (_dgc_source) {
        CGImageRef imageRef = CGImageSourceCreateImageAtIndex(_dgc_source, index, (CFDictionaryRef)@{(id)kCGImageSourceShouldCache:@(YES)});
    ...
    if (_dgc_apngSource) {
        uint32_t size = 0;
        uint8_t *bytes = yy_png_copy_frame_data_at_index(_data.bytes, _dgc_apngSource, (uint32_t)index, &size);
    ...
    if (_dgc_webpSource) {
        WebPIterator iter;
        if (!WebPDemuxGetFrame(_dgc_webpSource, (int)(index + 1), &iter)) return NULL; // demux webp frame data
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_dgc_blendCanvas) {
        _dgc_blendFrameIndex = NSNotFound;
        _dgc_blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, YYCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _dgc_blendCanvas != NULL;
    return suc;
}
...
- (void)_blendImageWithFrame:(_YYImageDecoderFrame *)frame {
    if (frame.dispose == YYImageDisposePrevious) {
        ...
        if (frame.blend == YYImageBlendOver) {
            ...
            if (unblendImage) {
                CGContextDrawImage(_dgc_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendImage);
...
- (CGImageRef)_newBlendedImageWithFrame:(_YYImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    ...
    if (frame.dispose == YYImageDisposePrevious) {
        if (frame.blend == YYImageBlendOver) {
            CGImageRef previousImage = CGBitmapContextCreateImage(_dgc_blendCanvas);
            CGImageRef unblendImage = [self _newUnblendedImageAtIndex:frame.index extendToCanvas:NO decoded:NULL];
            if (unblendImage) {
                CGContextDrawImage(_dgc_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height), unblendImage);
                CFRelease(unblendImage);
            ...
            CGContextClearRect(_dgc_blendCanvas, CGRectMake(0, 0, _width, _height));
            if (previousImage) {
                CGContextDrawImage(_dgc_blendCanvas, CGRectMake(0, 0, _width, _height), previousImage);
...
@implementation DGCYYImageEncoder {
    NSMutableArray *_dgc_images;
    NSMutableArray *_dgc_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"DGCYYImageEncoder init error" reason:@"DGCYYImageEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
...
- (instancetype)initWithType:(YYImageType)type {
    ...
    if (!self) return nil;
    _type = type;
    _dgc_images = [NSMutableArray new];
    _dgc_durations = [NSMutableArray new];
...
- (void)addImage:(UIImage *)image duration:(NSTimeInterval)duration {
    ...
    [_dgc_images addObject:image];
    [_dgc_durations addObject:@(duration)];
}

- (void)addImageWithData:(NSData *)data duration:(NSTimeInterval)duration {
    if (data.length == 0) return;
    duration = duration < 0 ? 0 : duration;
    [_dgc_images addObject:data];
    [_dgc_durations addObject:@(duration)];
}
...
- (BOOL)_imageIOAvaliable {
    switch (_type) {
        ...
        case YYImageTypeICNS:
        case YYImageTypeGIF: {
            return _dgc_images.count > 0;
        } break;
        case YYImageTypePNG: {
            return _dgc_images.count == 1;
        } break;
        case YYImageTypeWebP: {
            return NO;
        } break;
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            id imageSrc = _dgc_images[i];
            NSDictionary *frameProperty = NULL;
            if (_type == YYImageTypeGIF && count > 1) {
                frameProperty = @{(NSString *)kCGImagePropertyGIFDictionary : @{(NSString *) kCGImagePropertyGIFDelayTime:_dgc_durations[i]}};
            } else {
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == YYImageTypeGIF ? _dgc_images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _dgc_images.count; i++) {
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == YY_FOUR_CC('I', 'D', 'A', 'T')) {
            ...
            yy_png_delay_to_fraction([(NSNumber *)_dgc_durations[0] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _dgc_images.count; i++) {
    ...
    if (webpDatas.count == 1) {
        ...
        for (NSUInteger i = 0; i < _dgc_images.count; i++) {
            ...
            NSNumber *duration = _dgc_durations[i];
...
- (NSData *)encode {
    if (_dgc_images.count == 0) return nil;
...
- (BOOL)encodeToFile:(NSString *)path {
    if (_dgc_images.count == 0 || path.length == 0) return NO;
...
+ (NSData *)encodeImage:(UIImage *)image type:(YYImageType)type quality:(CGFloat)quality {
    DGCYYImageEncoder *encoder = [[DGCYYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(DGCYYImageDecoder *)decoder type:(YYImageType)type quality:(CGFloat)quality {
...
/// @param forSystem YES: used for system album (PNG/JPEG/GIF), NO: used for DGCYYImage (PNG/JPEG/GIF/WebP)
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[DGCYYImage class]]) {
        DGCYYImage *image = (id)self;
