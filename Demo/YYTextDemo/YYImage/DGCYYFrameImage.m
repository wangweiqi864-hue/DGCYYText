...
@implementation DGCYYFrameImage {
    NSUInteger _dgc_loopCount;
    NSUInteger _dgc_oneFrameBytes;
    NSArray *_dgc_imagePaths;
    NSArray *_dgc_imageDatas;
    NSArray *_dgc_frameDurations;
}
...
- (instancetype)initWithImagePaths:(NSArray *)paths frameDurations:(NSArray *)frameDurations loopCount:(NSUInteger)loopCount {
    ...
    _dgc_oneFrameBytes = (NSUInteger)frameByte;
    _dgc_imagePaths = paths.copy;
    _dgc_frameDurations = frameDurations.copy;
    _dgc_loopCount = loopCount;
...
- (instancetype)initWithImageDataArray:(NSArray *)dataArray frameDurations:(NSArray *)frameDurations loopCount:(NSUInteger)loopCount {
    ...
    long frameByte = CGImageGetBytesPerRow(firstCG.CGImage) * CGImageGetHeight(firstCG.CGImage);
    _dgc_oneFrameBytes = (NSUInteger)frameByte;
    _dgc_imageDatas = dataArray.copy;
    _dgc_frameDurations = frameDurations.copy;
    _dgc_loopCount = loopCount;
...
- (NSUInteger)animatedImageFrameCount {
    if (_dgc_imagePaths) {
        return _dgc_imagePaths.count;
    } else if (_dgc_imageDatas) {
        return _dgc_imageDatas.count;
    } else {
        return 1;
    }
}
...
- (NSUInteger)animatedImageLoopCount {
    return _dgc_loopCount;
}

- (NSUInteger)animatedImageBytesPerFrame {
    return _dgc_oneFrameBytes;
}

- (UIImage *)animatedImageFrameAtIndex:(NSUInteger)index {
    if (_dgc_imagePaths) {
        if (index >= _dgc_imagePaths.count) return nil;
        NSString *path = _dgc_imagePaths[index];
        CGFloat scale = _NSStringPathScale(path);
        NSData *data = [NSData dataWithContentsOfFile:path];
        return [[UIImage imageWithData:data scale:scale] yy_imageByDecoded];
    } else if (_dgc_imageDatas) {
        if (index >= _dgc_imageDatas.count) return nil;
        NSData *data = _dgc_imageDatas[index];
...
- (NSTimeInterval)animatedImageDurationAtIndex:(NSUInteger)index {
    if (index >= _dgc_frameDurations.count) return 0;
    NSNumber *num = _dgc_frameDurations[index];
