...
/**
 ...
 Sample Code:
     ...
     // ani@3x.gif
     DGCYYImage *image = [DGCYYImage imageNamed:@"ani"];
     DGCYYAnimatedImageView *imageView = [DGCYYAnimatedImageView alloc] initWithImage:image];
     [view addSubView:imageView];
 */
@interface DGCYYAnimatedImageView : UIImageView
...
/**
 ...
 display with DGCYYAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to DGCYYAnimatedImageView.image or DGCYYAnimatedImageView.highlightedImage
 to display animation.
 
 See `DGCYYImage` and `DGCYYFrameImage` for example.
 */
